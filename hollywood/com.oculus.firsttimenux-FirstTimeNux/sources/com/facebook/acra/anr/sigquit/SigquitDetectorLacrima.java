...
public class SigquitDetectorLacrima implements SigquitDetector {
    private static final String TAG = SigquitDetectorLacrima.class.getSimpleName();
    ...
    public static SigquitDetector getInstance(SigquitDetectorListener listener) {
        if (sInstance == null) {
            sInstance = new SigquitDetectorLacrima(listener);
        }
        return sInstance;
    }

    private SigquitDetectorLacrima(SigquitDetectorListener listener) {
    ...
    public void installSignalHandler(final Handler bkgThreadHandler, final boolean shouldStart) {
        final Runnable hookDetectorRunnable = new Runnable() {
            ...
            public void run() {
                boolean hookSuccessful = SigquitDetectorLacrima.nativeHookMethods();
                SigquitDetectorLacrima.this.mListener.onHookedMethods(hookSuccessful);
