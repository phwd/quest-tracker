...
final class NewProcReader implements IProcReader {
    ...
    public boolean parseProcLine(byte[] buffer, int startIndex, int endIndex, int[] format, @Nullable String[] outStrings, @Nullable long[] outLongs, @Nullable float[] outFloats) {
        ...
        int di = 0;
        for (int mode : format) {
            if ((mode & ProcReader.PROC_PARENS) != 0) {
                i++;
            } else if ((mode & 1024) != 0) {
                if (buffer[i] == 34) {
                    i++;
            ...
            char term = (char) (mode & ProcReader.PROC_TERM_MASK);
            if (i >= endIndex) {
            ...
            if ((mode & 28672) != 0) {
                ...
                if (!((mode & ProcReader.PROC_OUT_FLOAT) == 0 || di >= NR || outFloats == null)) {
                    outFloats[di] = ByteParse.strtof(buffer, i, null);
                }
                if (!((mode & ProcReader.PROC_OUT_LONG) == 0 || di >= NL || outLongs == null)) {
                    if ((mode & 2048) != 0) {
