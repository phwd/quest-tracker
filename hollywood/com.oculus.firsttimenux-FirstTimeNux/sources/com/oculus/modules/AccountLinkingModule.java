...
public class AccountLinkingModule extends AbstractBroadcastReceiverModule {
    ...
    public void unlink(final String serviceProvider, final int callbackID) {
        ...
        ThreadExecutor.getInstance().execute(new Callable<Integer>() {
            ...
            public void onFailure(@Nonnull Throwable throwable) {
                ...
                AccountLinkingModule.this.invokeCallback(callbackID, "");
            }
        });
    }
    ...
    public void facebookLinkWithCredentials(String emailOrPhone, String password, String tosVersion, String loggingExtrasJson, final int successCallbackID, final int failureCallbackID) {
        ...
        this.mOVRAuth.loginToFbAndLink(emailOrPhone, password, tosVersion, loggingExtrasJson, new AuthResultCallback<Void, AuthFbLoginError>() {
            ...
            public void onResult(Void result) {
                AccountLinkingModule.this.invokeCallback(successCallbackID, "");
            }
        ...
        this.mOVRAuth.checkFbMachineApproval(uid, machineID, new AuthResultCallback<Boolean, AuthError>() {
            ...
            public void onResult(Boolean result) {
                if (result.booleanValue()) {
                    ...
                    AccountLinkingModule.this.invokeCallback(failureCallbackID, "");
                }
            }
    ...
    public static JSONObject errorToJson(AuthError error) {
        try {
            ...
            resultJson.put(ServiceContract.EXTRA_ERROR_TITLE, error.getErrorTitle());
            resultJson.put(ServiceContract.EXTRA_ERROR_MESSAGE, error.getMessage());
            resultJson.put(ServiceContract.EXTRA_ERROR_CODE, error.getErrorCode());
            return resultJson;
        } catch (JSONException e) {
            Log.e(MODULE_NAME, "JSON Error", e);
    ...
    public static JSONObject fbLoginErrorToJson(AuthFbLoginError error) {
        try {
            ...
            resultJson.put("uid", nonNullElse(error.getUid(), ""));
            resultJson.put(ServiceContract.EXTRA_MACHINE_ID, nonNullElse(error.getMachineId(), ""));
            resultJson.put(ServiceContract.EXTRA_AUTH_TOKEN, nonNullElse(error.getAuthToken(), ""));
            resultJson.put(ServiceContract.EXTRA_LOGIN_FIRST_FACTOR, nonNullElse(error.getLoginFirstFactor(), ""));
            return resultJson;
        } catch (JSONException e) {
            Log.e(MODULE_NAME, "JSON Error", e);
            return new JSONObject();
    ...
    private static <T> T nonNullElse(@Nullable T value, T elseValue) {
