...
public final class Util {
    ...
    public static boolean skipAll(Source source, int duration, TimeUnit timeUnit) throws IOException {
        ...
        try {
            ...
            while (source.read(skipBuffer, Systrace.TRACE_TAG_REACT_JAVA_BRIDGE) != -1) {
                skipBuffer.clear();
            }
    ...
    private static InetAddress decodeIpv6(String input, int pos, int limit) {
        ...
        while (true) {
            if (i >= limit) {
                ...
                int b2 = b + 1;
                address[b] = (byte) ((value >>> 8) & ProcReader.PROC_TERM_MASK);
                b = b2 + 1;
                address[b2] = (byte) (value & ProcReader.PROC_TERM_MASK);
            }
        }
        if (b != address.length) {
            if (compress == -1) {
    ...
    private static String inet6AddressToAscii(byte[] address) {
        ...
        while (i2 < address.length) {
            if (i2 == longestRunOffset) {
                ...
                result.writeHexadecimalUnsignedLong((long) (((address[i2] & UnsignedBytes.MAX_VALUE) << 8) | (address[i2 + 1] & UnsignedBytes.MAX_VALUE)));
                i2 += 2;
            }
        }
