...
public class WebpUtil {
    ...
    private static int getInt(InputStream inputStream) throws IOException {
        return ((((byte) inputStream.read()) << Ascii.CAN) & ViewCompat.MEASURED_STATE_MASK) | ((((byte) inputStream.read()) << Ascii.DLE) & 16711680) | ((((byte) inputStream.read()) << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (((byte) inputStream.read()) & UnsignedBytes.MAX_VALUE);
    }

    public static int get2BytesAsInt(InputStream inputStream) throws IOException {
        return ((((byte) inputStream.read()) << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (((byte) inputStream.read()) & UnsignedBytes.MAX_VALUE);
    }

    private static int read3Bytes(InputStream inputStream) throws IOException {
        byte b = getByte(inputStream);
        return ((getByte(inputStream) << Ascii.DLE) & 16711680) | ((getByte(inputStream) << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (b & UnsignedBytes.MAX_VALUE);
    }
