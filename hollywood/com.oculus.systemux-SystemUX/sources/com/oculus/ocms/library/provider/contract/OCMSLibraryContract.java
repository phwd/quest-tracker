...
public class OCMSLibraryContract {
    ...
    public static Uri uriForAllPackages() {
        return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path("apps").build();
    }

    public static Uri uriForPackage(String str) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path("apps").appendPath(str).build();
        }
        throw new IllegalArgumentException("packageName cannot be null/empty");
    }

    public static Uri uriForAllDownloads() {
        return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path(DOWNLOAD_PATH).build();
    }
    ...
    public static Uri uriForPackageDownload(String str, int i, long j) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path(DOWNLOAD_PATH).appendPath(str).appendPath(Integer.toString(i)).appendPath(Long.toString(j)).build();
        }
        throw new IllegalArgumentException("install cannot be null/empty");
    }

    public static Uri uriForAllInstalls() {
        return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path(INSTALL_PATH).build();
    }
    ...
    public static Uri uriForPackageInstall(String str, int i, long j) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path(INSTALL_PATH).appendPath(str).appendPath(Integer.toString(i)).appendPath(Long.toString(j)).build();
        }
    ...
    public static Uri getAllAssetsUri(String str) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path("assets").appendPath(str).build();
        }
        throw new IllegalArgumentException("packageName cannot be null/empty");
    }
    ...
    public static Uri getAssetByIdUri(String str, long j) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path("assets").appendPath(str).appendPath("id").appendPath(Long.toString(j)).build();
        }
        throw new IllegalArgumentException("packageName cannot be null/empty");
    }
    ...
    public static Uri getAssetByFilenameUri(String str, String str2) {
        if (TextUtils.isEmpty(str)) {
            ...
            return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path("assets").appendPath(str).appendPath(ASSETS_PATH_BY_FILENAME).appendPath(str2).build();
        } else {
    ...
    public static Uri queryForDeploymentGk() {
        return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(AUTHORITY).path(DEPLOYMENT_PATH).build();
    }
}
