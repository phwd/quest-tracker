...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public void onMeasure(int i, int i2) {
        ...
        if (this.mFillViewport && View.MeasureSpec.getMode(i2) != 0 && getChildCount() > 0) {
            ...
            if (measuredHeight < measuredHeight2) {
                childAt.measure(getChildMeasureSpec(i, getPaddingLeft() + getPaddingRight() + layoutParams.leftMargin + layoutParams.rightMargin, layoutParams.width), View.MeasureSpec.makeMeasureSpec(measuredHeight2, Ints.MAX_POWER_OF_TWO));
            }
        }
    }
            ...
            if (actionMasked == 1) {
                ...
                if (findPointerIndex == -1) {
                    ...
                    if (this.mIsBeingDragged) {
                        ...
                        iArr[1] = 0;
                        dispatchNestedScroll(0, scrollY2, 0, i3, this.mScrollOffset, 0, iArr);
                        int i4 = this.mLastMotionY;
                        int[] iArr2 = this.mScrollOffset;
                        this.mLastMotionY = i4 - iArr2[1];
                        this.mNestedYOffset += iArr2[1];
                        if (z) {
                            int i5 = i2 - this.mScrollConsumed[1];
                            ensureGlows();
                            int i6 = scrollY + i5;
                            if (i6 < 0) {
                                EdgeEffectCompat.onPull(this.mEdgeGlowTop, ((float) i5) / ((float) getHeight()), motionEvent.getX(findPointerIndex) / ((float) getWidth()));
                                if (!this.mEdgeGlowBottom.isFinished()) {
                                    this.mEdgeGlowBottom.onRelease();
                                }
                            } else if (i6 > scrollRange) {
                                EdgeEffectCompat.onPull(this.mEdgeGlowBottom, ((float) i5) / ((float) getHeight()), 1.0f - (motionEvent.getX(findPointerIndex) / ((float) getWidth())));
    ...
    public void fling(int i) {
        if (getChildCount() > 0) {
            this.mScroller.fling(getScrollX(), getScrollY(), 0, i, 0, 0, Integer.MIN_VALUE, BytesRange.TO_END_OF_CONTENT, 0, 0);
