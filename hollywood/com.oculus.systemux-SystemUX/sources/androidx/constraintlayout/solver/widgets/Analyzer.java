...
public class Analyzer {
    ...
    private static int getMaxDimensionTraversal(ConstraintWidget constraintWidget, int i, boolean z, int i2) {
        ...
        while (it.hasNext()) {
            i12 = Math.max(i12, getMaxDimensionTraversal(((ResolutionAnchor) ((ResolutionNode) it.next())).myAnchor.mOwner, i, z, i14));
        }
        int i15 = 0;
        for (Iterator it2 = constraintWidget.mListAnchors[i7].getResolutionNode().dependents.iterator(); it2.hasNext(); it2 = it2) {
            i15 = Math.max(i15, getMaxDimensionTraversal(((ResolutionAnchor) ((ResolutionNode) it2.next())).myAnchor.mOwner, i, z, width2 + i14));
        }
        if (z2) {
            i12 -= i6;
            width = i15 + i5;
