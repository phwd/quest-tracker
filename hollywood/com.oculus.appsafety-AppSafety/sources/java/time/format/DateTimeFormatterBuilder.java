...
public final class DateTimeFormatterBuilder {
    ...
    public static final class LocalizedOffsetIdPrinterParser implements DateTimePrinterParser {
        ...
        /* JADX INFO: Multiple debug info for r4v2 int: [D('h2' int), D('pos' int)] */
        /* JADX INFO: Multiple debug info for r1v11 int: [D('h' int), D('pos' int)] */
        @Override // java.time.format.DateTimeFormatterBuilder.DateTimePrinterParser
        public int parse(DateTimeParseContext context, CharSequence text, int position) {
            int negative;
            int pos;
            ...
            if (this.style == TextStyle.FULL) {
                ...
                if (h1 >= 0 && h2 >= 0) {
                    ...
                    if (text.charAt(pos6) == 58) {
                        ...
                        int pos9 = pos8 + 1;
                        int m2 = getDigit(text, pos8);
                        if (m12 >= 0) {
                            if (m2 >= 0) {
                                int m3 = (m12 * 10) + m2;
                                if (pos9 + 2 < end) {
                                    if (text.charAt(pos9) == ':') {
                                        int s1 = getDigit(text, pos9 + 1);
                                        int s2 = getDigit(text, pos9 + 2);
                                        if (s1 >= 0 && s2 >= 0) {
                                            s = (s1 * 10) + s2;
                                            pos9 += 3;
                                        }
            ...
            if (pos10 < end) {
                int h22 = getDigit(text, pos10);
                if (h22 >= 0) {
                    pos10++;
                    h = (h * 10) + h22;
                }
                if (pos10 + 2 < end && text.charAt(pos10) == ':' && pos10 + 2 < end && text.charAt(pos10) == ':') {
                    int m13 = getDigit(text, pos10 + 1);
                    int m22 = getDigit(text, pos10 + 2);
                    if (m13 >= 0 && m22 >= 0) {
                        m = (m13 * 10) + m22;
                        pos10 += 3;
                        if (pos10 + 2 < end && text.charAt(pos10) == ':') {
                            int s12 = getDigit(text, pos10 + 1);
                            int s22 = getDigit(text, pos10 + 2);
                            if (s12 >= 0 && s22 >= 0) {
                                m1 = m;
                                pos2 = (s12 * 10) + s22;
                                pos = pos10 + 3;
