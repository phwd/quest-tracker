...
public class BigInteger extends Number implements Comparable<BigInteger>, Serializable {
    ...
    public static BigInteger valueOf(long value) {
        if (value >= 0) {
            ...
            if (value < ((long) bigIntegerArr.length)) {
                return bigIntegerArr[(int) value];
            }
    ...
    public long longValue() {
        ...
        if (this.numberLength > 1) {
            ...
            value = (((long) iArr[0]) & 4294967295L) | (((long) iArr[1]) << 32);
        } else {
    ...
    /* JADX INFO: Multiple debug info for r1v2 int: [D('highBytes' int), D('hB' int)] */
    private byte[] twosComplement() {
        ...
        while (bytesLen2 > firstByteNumber) {
            int digit3 = this.digits[digitIndex];
            digitIndex++;
    ...
    private static void parseFromString(BigInteger bi, String value, int radix) {
        ...
        while (substrStart < stringLength) {
            digits2[digitIndex] = multiplyByInt(digits2, digits2, digitIndex, bigRadix) + inplaceAdd(digits2, digitIndex, Integer.parseInt(value.substring(substrStart, substrEnd), radix));
            substrStart = substrEnd;
            substrEnd = substrStart + charsPerInt;
            digitIndex++;
        }
        bi.setJavaRepresentation(sign2, digitIndex, digits2);
    }
    ...
    public BigInteger copy() {
        ...
        int[] copyDigits = new int[i];
        System.arraycopy((Object) this.digits, 0, (Object) copyDigits, 0, i);
