...
public class SignatureFileVerifier {
    ...
    private boolean verifySection(Attributes sfAttr, String name2, ManifestDigester md2) throws IOException, SignatureException {
            ...
            if (sfAttr != null) {
                for (Map.Entry<Object, Object> se : sfAttr.entrySet()) {
                    ...
                    if (key.toUpperCase(Locale.ENGLISH).endsWith("-DIGEST") && (digest = getDigest(key.substring(0, key.length() - 7))) != null) {
                        ...
                        Debug debug2 = debug;
                        if (debug2 != null) {
                            debug2.println("Signature Block File: " + name2 + " digest=" + digest.getAlgorithm());
                            Debug debug3 = debug;
                            StringBuilder sb = new StringBuilder();
                            sb.append("  expected ");
                            sb.append(toHex(expected));
        ...
        throw new SecurityException("no manifest section for signature file entry " + name2);
    }
