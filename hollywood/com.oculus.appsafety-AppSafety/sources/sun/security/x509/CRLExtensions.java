...
public class CRLExtensions {
    ...
    private void init(DerInputStream derStrm) throws CRLException {
        ...
        try {
            ...
            if ((nextByte & DerValue.TAG_PRIVATE) == 128 && (nextByte & 31) == 0) {
                str = str.getDerValue().data;
            }
            for (DerValue derValue : str.getSequence(5)) {
    ...
    public boolean equals(Object other) {
        ...
        if (!((other instanceof CRLExtensions) && (len = (objs = ((CRLExtensions) other).getAllExtensions().toArray()).length) == this.map.size())) {
            return false;
        }
        String key = null;
        for (int i = 0; i < len; i++) {
