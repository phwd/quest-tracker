        ...
        if (this.hasCursor) {
            ...
            if (i < 0) {
                ...
                while (n < 0 && newStart3 > 0) {
                    newStart3 -= UTF16.getCharCount(text.char32At(newStart3 - 1));
                    n++;
                }
                newStart = newStart3 + n;
            } else if (i > this.output.length()) {
                int newStart4 = start + outLen;
                int n2 = this.cursorPos - this.output.length();
                while (n2 > 0 && newStart4 < text.length()) {
                    newStart4 += UTF16.getCharCount(text.char32At(newStart4));
                    n2--;
                }
