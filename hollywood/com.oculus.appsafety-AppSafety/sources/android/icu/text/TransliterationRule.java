...
public class TransliterationRule {
    ...
    public TransliterationRule(String input, int anteContextPos, int postContextPos, String output2, int cursorPos, int cursorOffset, UnicodeMatcher[] segs, boolean anchorStart, boolean anchorEnd, RuleBasedTransliterator.Data theData) {
        ...
        if (i > 0) {
            this.anteContext = new StringMatcher(this.pattern.substring(0, i), 0, this.data);
        }
        this.key = null;
        ...
        if (i2 > 0) {
            ...
            this.key = new StringMatcher(str.substring(i3, i2 + i3), 0, this.data);
        }
        int length = this.pattern.length();
        int i4 = this.keyLength;
        int i5 = this.anteContextLength;
        int postContextLength = (length - i4) - i5;
        this.postContext = null;
        if (postContextLength > 0) {
            this.postContext = new StringMatcher(this.pattern.substring(i5 + i4), 0, this.data);
        }
    ...
    /* JADX INFO: Multiple debug info for r3v3 int: [D('oText' int), D('match' int)] */
    /* JADX INFO: Multiple debug info for r7v4 int: [D('match' int), D('keyLimit' int)] */
