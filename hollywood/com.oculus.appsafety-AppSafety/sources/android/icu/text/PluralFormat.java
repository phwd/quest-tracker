...
public class PluralFormat extends UFormat {
    ...
    /* JADX INFO: Multiple debug info for r5v4 int: [D('type' android.icu.text.MessagePattern$Part$Type), D('partIndex' int)] */
        ...
        while (true) {
            ...
            if (part2.getType() != MessagePattern.Part.Type.ARG_LIMIT) {
                if (pattern2.getPartType(partIndex3).hasNumericValue()) {
                    ...
                    if (!pattern2.partSubstringMatches(part2, PluralRules.KEYWORD_OTHER)) {
                        if (keyword == null) {
                ...
                if (pluralFormat.msgPattern.getPart(partIndex).getType() != MessagePattern.Part.Type.ARG_SELECTOR) {
                    ...
                    if (partStart.getType() != MessagePattern.Part.Type.MSG_START) {
                        ...
                        if (partLimit.getType() != MessagePattern.Part.Type.MSG_LIMIT) {
                            ...
                            if (scanner != null) {
                                currMatchIndex = scanner.findText(str2, currArg, startingAt)[0];
                            } else {
    ...
    public String toString() {
        ...
        buf.append("locale=" + ((Object) this.ulocale));
        buf.append(", rules='" + ((Object) this.pluralRules) + "'");
        buf.append(", pattern='" + this.pattern + "'");
        buf.append(", format='" + ((Object) this.numberFormat) + "'");
        return buf.toString();
    }
