...
public class IndianCalendar extends Calendar {
    ...
    /* JADX INFO: Multiple debug info for r9v2 int: [D('mday' int), D('IndianMonth' int)] */
    /* access modifiers changed from: protected */
    @Override // android.icu.util.Calendar
    public void handleComputeFields(int julianDay) {
        ...
        if (yday < leapMonth) {
            ...
            int IndianMonth3 = yday - leapMonth;
            if (IndianMonth3 < 155) {
                IndianMonth = (IndianMonth3 / 31) + 1;
                IndianMonth2 = (IndianMonth3 % 31) + 1;
            } else {
                int mday = IndianMonth3 - 155;
                IndianMonth = (mday / 30) + 6;
                IndianMonth2 = (mday % 30) + 1;
            }
        }
        internalSet(0, 0);
    ...
    private static double gregorianToJD(int year, int month, int date) {
        int y = year - 1;
        return ((double) ((((((((y * 365) + (y / 4)) - (y / 100)) + (y / 400)) + (((month * 367) - 362) / 12)) + (month <= 2 ? 0 : isGregorianLeap(year) ? -1 : -2)) + date) - 1)) + 1721425.5d;
    }
    ...
    private static int[] jdToGregorian(double jd) {
        ...
        int year = (int) ((400.0d * quadricent) + (100.0d * cent) + (quad * 4.0d) + yindex);
