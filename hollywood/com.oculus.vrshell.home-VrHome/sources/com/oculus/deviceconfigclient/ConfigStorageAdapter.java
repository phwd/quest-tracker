...
class ConfigStorageAdapter {
        ...
        for (StorageParam param : internalRepresentation.Params) {
            ...
            try {
                switch (paramType.hashCode()) {
                    case -1325958191:
                        if (paramType.equals(Constants.DOUBLE_TYPE_JSON_VALUE)) {
                            c = 1;
                            break;
                        }
                        break;
                    case -891985903:
                        if (paramType.equals(Constants.STRING_TYPE_JSON_VALUE)) {
                            c = 3;
                            break;
                        }
                        break;
                    case 3327612:
                        if (paramType.equals(Constants.LONG_TYPE_JSON_VALUE)) {
                            c = 2;
                            break;
                        }
                        break;
                    case 64711720:
                        if (paramType.equals(Constants.BOOLEAN_TYPE_JSON_VALUE)) {
                            c = 0;
                            break;
                        }
                        break;
    ...
    public static void writeMemoryStateToStorageCache(Context context, ConfigMemoryState memoryState) {
        ...
        writeValues(internalRepresentation, Constants.BOOLEAN_TYPE_JSON_VALUE, memoryState.getBooleanValues());
        writeValues(internalRepresentation, Constants.DOUBLE_TYPE_JSON_VALUE, memoryState.getDoubleValues());
        writeValues(internalRepresentation, Constants.LONG_TYPE_JSON_VALUE, memoryState.getLongValues());
        writeValues(internalRepresentation, Constants.STRING_TYPE_JSON_VALUE, memoryState.getStringValues());
