...
public class QuickPromotionModuleImpl extends QuickPromotionModule {
    ...
    public void initializeImpl(QuickPromotionModule.QPApplication application, boolean enabled, boolean shouldSendActionGraphQL, boolean shouldSendExposureGraphQL, boolean shouldSendImpressionGraphQL, Resolver<Void> resolver) {
        Log.d(TAG, "Initialized and " + (enabled ? "" : "not ") + "enabled.");
    ...
    private String signalTrigger(String trigger) {
                    ...
                    if ((1000 * ((long) promotionEdge.optInt("client_ttl_seconds"))) + cacheTimestamp >= System.currentTimeMillis()) {
                        ...
                        if (currentTime >= start && end >= currentTime) {
                            ...
                            try {
                                ...
                                promotion.put(QuickPromotionGraphQL.ARGUMENT_SURFACE_ID, surfaceID);
        ...
        try {
            ...
            eventData.put(QuickPromotionLogEvent.KEY_TRIGGER, trigger);
    ...
    public void markPromotionExposedHelper(QuickPromotionModule.QPPromotion promotion, final String trigger, Resolver<Void> resolver, final boolean is_holdout) {
        ...
        if (is_holdout) {
            QuickPromotionLogging.logPromotionHoldoutExposure(promotionID, surfaceID, trigger);
        } else {
            QuickPromotionLogging.logPromotionExposure(promotionID, surfaceID, trigger);
        }
        if (this.mAccessToken != null && mShouldSendExposureGraphQL) {
            GraphQLUtil.query(getHttpClient(), QuickPromotionGraphQL.LOG_EVENT_MUTATION, this.mAccessToken, QuickPromotionGraphQL.getParamsForExposure(promotionID, logData, surfaceID, trigger), new GraphQLUtil.Callback() {
    ...
    public void markPromotionShownImpl(QuickPromotionModule.QPPromotion promotion, final String trigger, Resolver<Void> resolver) {
        ...
        final String promotionID = getIDForPromotion(promotion);
        final String logData = getLoggingDataForPromotion(promotion);
        QuickPromotionLogging.logPromotionImpression(promotionID, surfaceID, trigger);
        if (this.mAccessToken != null && mShouldSendImpressionGraphQL) {
            GraphQLUtil.query(getHttpClient(), QuickPromotionGraphQL.LOG_EVENT_MUTATION, this.mAccessToken, QuickPromotionGraphQL.getParamsForView(promotionID, logData, surfaceID, trigger), new GraphQLUtil.Callback() {
    ...
    public void markPromotionActionImpl(QuickPromotionModule.QPPromotion promotion, final String trigger, final String action, Resolver<Void> resolver) {
        ...
        final String promotionID = getIDForPromotion(promotion);
        final String logData = getLoggingDataForPromotion(promotion);
        QuickPromotionLogging.logPromotionAction(action, promotionID, surfaceID, trigger);
        if (this.mAccessToken != null && mShouldSendActionGraphQL) {
            GraphQLUtil.query(getHttpClient(), QuickPromotionGraphQL.LOG_EVENT_MUTATION, this.mAccessToken, QuickPromotionGraphQL.getParamsForAction(promotionID, logData, surfaceID, trigger, action), new GraphQLUtil.Callback() {
    ...
    private JSONObject getPromotionsForSurface(int surfaceID, String trigger) {
        ...
        try {
            ...
            String cacheLocale = promotions.getString(OculusContent.Profile.LOCALE);
    ...
    private void fetchForSurface(OkHttpClient client, final int surfaceID, final String trigger, final SurfaceFetchCallback callback) {
        ...
        try {
            ...
            queryParams.put(QuickPromotionGraphQL.ARGUMENT_SURFACE_ID, surfaceID);
        } catch (JSONException e) {
            Log.e(TAG, "Json error: " + e.getMessage());
            queryParams = null;
        }
        if (this.mAccessToken != null) {
            GraphQLUtil.query(client, QuickPromotionGraphQL.QUICK_PROMOTION_QUERY, this.mAccessToken, queryParams, new GraphQLUtil.Callback() {
    ...
    private void storeListInSharedPreferences(int surfaceID, JSONArray promotions) {
        ...
        try {
            ...
            promotionsToStore.put(OculusContent.Profile.LOCALE, this.mLocale);
