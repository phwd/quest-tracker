...
public abstract class GraphQLPrefetch extends RCTBaseJavaModule implements AccessTokenForwarder.IListener, PanelEnvironment.Listener {
    ...
    private final void pushPrefetchQueries() {
        try {
            ...
            for (GraphQLPrefetchQuery query : this.mQueries) {
                array.put(new JSONObject().put("docId", String.valueOf(query.getDocId())).put(ApiConstants.LATEST_SYNTAX_GRAPHQL_QUERY_PARAMS_KEY, query.getVariables()));
            }
            ...
            nativeEmitEventWithJsonData(this.RVRCtxTag, "onGetPrefetchQueries", XMPConst.ARRAY_ITEM_NAME);
        }
    }
    ...
    private String getJSONString(long docId, JSONObject variables, JSONObject response) {
        ...
        try {
            ...
            obj.put(ApiConstants.LATEST_SYNTAX_GRAPHQL_QUERY_PARAMS_KEY, variables);
            if (response != null) {
                obj.put(OculusContent.FriendList.GROUP_LAUNCH_RESPONSE_PARAM, response);
            }
            return obj.toString();
        } catch (JSONException e) {
