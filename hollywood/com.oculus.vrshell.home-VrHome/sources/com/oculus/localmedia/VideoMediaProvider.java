...
public class VideoMediaProvider {
    private static final String CINEMAGRAPH_FILENAME = "cinemagraph.mp4";
    private static final String DOWNLOADED_MOVIES_PATH = "Download/oculus_downloaded_videos";
    ...
    private void scanMediaStore(ArrayList<MediaItem> items) {
        ...
        while (cursor2.moveToNext()) {
            try {
                ...
                if (filePath.indexOf(DOWNLOADED_MOVIES_PATH) > 0) {
                    Log.d(LocalMediaManager.TAG, "Skipping downloaded movie : " + filePath);
                } else if (filePath.toLowerCase().indexOf(CINEMAGRAPH_FILENAME) <= 0) {
        ...
        if (!(path == null || !path.isDirectory() || path.listFiles() == null)) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (!file.isDirectory() || !recursive) {
                    ...
                    if (filePath.toLowerCase().indexOf(CINEMAGRAPH_FILENAME) <= 0) {
