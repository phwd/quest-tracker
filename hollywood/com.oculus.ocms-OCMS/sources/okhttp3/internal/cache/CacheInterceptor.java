...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request == null && response2 == null) {
            return new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(FragmentConstants.ContentFragmentType.GROUPS_POST_TAG_CONSUMPTION_FRAGMENT).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1).receivedResponseAtMillis(System.currentTimeMillis()).build();
        }
        if (request == null) {
    ...
    private static Headers combine(Headers headers, Headers headers2) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((!com.google.common.net.HttpHeaders.WARNING.equalsIgnoreCase(name) || !value.startsWith(ActivityConstants.Extras.WATCH_FEED_INJECTION)) && (!isEndToEnd(name) || headers2.get(name) == null)) {
