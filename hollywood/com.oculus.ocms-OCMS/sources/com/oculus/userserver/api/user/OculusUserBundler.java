...
public final class OculusUserBundler {
    ...
    public static Bundle bundleUser(OculusUser oculusUser) {
        ...
        bundle.putBoolean(KEY_IS_SUSPENDED, oculusUser.isSuspended());
        return bundle;
    }
    ...
    public static OculusUser unbundleUser(Bundle bundle) {
        ...
        } else if (bundle.containsKey(KEY_IS_SUSPENDED)) {
            return new OculusUser(bundle.getInt("user_id"), bundle.getString("user_name"), bundle.getString(KEY_PICTURE_URI), bundle.getLong(KEY_CREATION_TIME), bundle.getBoolean(KEY_IS_SUSPENDED));
        } else {
            throw new IllegalArgumentException("is_suspended must not be null");
        }
    }
