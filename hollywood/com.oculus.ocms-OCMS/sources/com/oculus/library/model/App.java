...
public class App {
    ...
    public static Builder forTesting() {
        return new Builder().withId("id").withPackageName("pkg").withStatus(AppStatus.ENTITLED).withDisplayName("name").withComfortRating(ComfortRating.COMFORTABLE_FOR_ALL).withImages(Collections.EMPTY_MAP).withCategory(Category.APPS).withDownloadSizeBytes(0).withDownloadedSizeBytes(0).withVersion(1, ActivityConstants.Extras.WATCH_FEED_INJECTION).withLatestVersion(2, "2").withLatestTargetSdkVersion(25).withPermission(Collections.emptyList()).withSupportedDevices(Collections.emptyList()).withInstallableExternalStorage(false).withEntitlementHash("fake").withAppScopedUserId("scoped").withApplicationGroupingId("group-id").withApplicationOrganizationId("organization-id").withMicrophoneUsage(MicrophoneUsage.NONE).withMinRecommendedVersionCode(2).withMinRequiredVersionCode(1).withIsDemoOf("pkg").withPlatform(SupportedPlatform.ANDROID_6DOF).withHeadTracking(HeadTracking.REQUIRE_6DOF).withAppMediaCapabilities(Collections.singletonList(AppMediaCapability.SCREENRECORDING)).withPlayMode(Collections.singletonList(PlayMode.ROOM_SCALE)).withCloudStorageStatus(CloudStorageStatus.DISABLED).withIsUnseen(false).withRecentActivityMs(0).withTotalActivityMs(0).withGrantReason(GrantReason.OCULUS_EMPLOYEE_OFFER).withGrantTimeMs(1).withGrantExpirationMs(1000).withApkSize(0).withCurrentLanguagePack("en-US").withInternetConnection("interwebs").withTrustedBinaryStatus(TrustedBinaryContract.STATUS_TRUSTED).withIsConcept(false).withIsTVApp(false).withIsTest(false).withSystemUiAsOverlayMode("ENABLED").withAutoUpdateEnabled(false).withDucNonCompliant(false).withAccessFeature(Collections.emptyList()).withEnvironmentSourceId("env-src-id").withEnvironmentSourceDisplayName("env-src-display-name");
    }
