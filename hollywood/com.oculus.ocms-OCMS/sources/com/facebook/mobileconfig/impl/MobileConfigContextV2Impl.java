...
public class MobileConfigContextV2Impl extends MobileConfigContextBase implements MobileConfigContext {
    private static final int SOFT_ERROR_FREQUENCY = 100000;
    private static final String TAG = "MobileConfigContextV2Impl";
    ...
    @Nullable
    public byte[] getBoolsBuffer() {
        FBMobileConfigTableOptimized fBMobileConfigTableOptimized = this.mConfigTable;
        if (fBMobileConfigTableOptimized == null) {
            return null;
        }
        try {
            ByteBuffer boolsAsByteBuffer = fBMobileConfigTableOptimized.boolsAsByteBuffer();
            byte[] bArr = new byte[boolsAsByteBuffer.remaining()];
            boolsAsByteBuffer.get(bArr);
            return bArr;
    ...
    public int getLoggingModeWithoutCaptureStack(long j) {
        ...
        return MobileConfigTableUtil.getLoggingModeWithoutCaptureStackFromMeta(getMeta(j));
    }
    ...
    public int getLoggingMode(long j) {
        ...
        return MobileConfigTableUtil.getLoggingModeFromMeta(getMeta(j));
    }
    ...
    public String getLoggingId(long j) {
        ...
        int meta = getMeta(j);
        try {
            int loggingSlotIdFromMeta = MobileConfigTableUtil.hasLoggingIdFromMeta(meta) ? MobileConfigTableUtil.getLoggingSlotIdFromMeta(meta) : -1;
            if (loggingSlotIdFromMeta >= 0) {
                return this.mConfigTable.loggingIds(loggingSlotIdFromMeta);
            }
            return null;
    ...
    public Map<Integer, Integer> getEmergencyPushInfo() {
        ...
        return fBMobileConfigTableOptimized.getEmergencyPushInfo();
    }

    public void clearRateLimiters() {
        this.mAutoExposureRateLimiter.clear();
        this.mManualExposureRateLimiter.clear();
    }
    ...
    public String getSchemaHash() {
        FBMobileConfigTableOptimized fBMobileConfigTableOptimized = this.mConfigTable;
        if (fBMobileConfigTableOptimized != null) {
            return fBMobileConfigTableOptimized.schemaHash();
        }
        return null;
    }

    public boolean shouldReturnNull(long j) {
        if (hasOverride(j)) {
            return false;
        }
        if (this.mConfigTable == null || MobileConfigSpecifierUtil.getParamType(j) != 3) {
            return true;
        }
        try {
            return MobileConfigTableUtil.isNullFromMeta(getMeta(j));
    ...
    @VisibleForTesting
    public void setManualExposureRateLimiter(ExposureRateLimiter exposureRateLimiter) {
        this.mManualExposureRateLimiter = exposureRateLimiter;
    }

    @VisibleForTesting
    public void setAutoExposureRateLimiter(ExposureRateLimiter exposureRateLimiter) {
        this.mAutoExposureRateLimiter = exposureRateLimiter;
    }

    public long getRequestTimestamp() {
        FBMobileConfigTableOptimized fBMobileConfigTableOptimized = this.mConfigTable;
        if (fBMobileConfigTableOptimized != null) {
            return fBMobileConfigTableOptimized.requestTimestamp();
        }
        return -1;
    }
    ...
    private void logError(String str) {
        ...
        if (fbErrorReporter != null) {
            fbErrorReporter.softReport(TAG, str, SOFT_ERROR_FREQUENCY);
        }
    }
}
