...
public class FbInjectorImpl extends AbstractInjector implements ScopeUnawareInjector, StaticBindingInterceptor {
    ...
    public void updateWithModules(List<? extends PrivateModule> list) {
        ensureInitialized();
        FbInjectorInitializer.Result update = new FbInjectorInitializer(this, list, this.mIsDebugMode, this.mBindingMap, this.mScopeMap, this.mComponentBindingMap, this.mModulesInstalled, this.mMultiBindings).update();
        this.mBindingMap = update.bindingMap;
        this.mScopeMap = update.scopeMap;
        this.mComponentBindingMap = update.componentBindingMap;
        this.mModulesInstalled = update.modulesInstalled;
        this.mMultiBindings = update.multiBindings;
    }
    ...
    public Object getInstance(int i) {
        return getInstance(i, getInjectorThreadStack().getContext());
    }
    ...
    public Object getInstance(int i, Context context) {
        ...
        if (this.mBindingMap.containsKey(Integer.valueOf(i))) {
            return ((Binding) Assertions.assertNotNull(this.mBindingMap.get(Integer.valueOf(i)))).getProvider().get();
        }
        Key keyFromId = getKeyFromId(i);
        StringBuilder sb = new StringBuilder();
        sb.append("No provider bound for ");
        if (keyFromId.getAnnotation() != null) {
            str = "@" + keyFromId.getAnnotation() + " ";
        } else {
            str = "";
        }
        sb.append(str);
        sb.append(keyFromId.getTypeLiteral());
        sb.append(", Map has # bindings: ");
        sb.append(this.mBindingMap.size());
    ...
    private Key getKeyFromId(int i) {
        Key intToKey = UL.id.intToKey(i);
        return intToKey == null ? RuntimeBindingIdUtils.bindingIdToKey(i) : intToKey;
    }
