...
public class OCMSLibraryContract {
    ...
    public static Uri uriForPackageDownload(String str, int i, long j) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme("content").authority(AUTHORITY).path(DOWNLOAD_PATH).appendPath(str).appendPath(Integer.toString(i)).appendPath(Long.toString(j)).build();
        }
