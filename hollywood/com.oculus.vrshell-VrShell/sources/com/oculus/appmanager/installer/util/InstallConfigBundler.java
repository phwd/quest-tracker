...
public class InstallConfigBundler {
    private static final String TAG = "InstallConfigBundler";
    ...
    public static Bundle toBundle(List<UpdateConfig> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            Bundle bundle2 = toBundle(list.get(i));
            bundle.putBundle(UpdateConfigContract.KEY_BUNDLE_ARRAY_PREFIX + i, bundle2);
        }
        return bundle;
    }
    ...
    public static List<UpdateConfig> toConfigList(Bundle bundle) {
        ...
        int i = 0;
        while (true) {
            if (!bundle.containsKey(UpdateConfigContract.KEY_BUNDLE_ARRAY_PREFIX + i)) {
                return arrayList;
            }
