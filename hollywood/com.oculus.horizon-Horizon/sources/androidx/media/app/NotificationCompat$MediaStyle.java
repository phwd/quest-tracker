...
public class NotificationCompat$MediaStyle extends AnonymousClass03p {
    ...
    private RemoteViews generateMediaActionButton(AnonymousClass03d r5) {
        boolean z = false;
        if (r5.A01 == null) {
            z = true;
        }
        RemoteViews remoteViews = new RemoteViews(this.mBuilder.A0B.getPackageName(), (int) R.layout.notification_media_action);
    ...
    public RemoteViews generateBigContentView() {
        int min = Math.min(this.mBuilder.A0F.size(), 5);
        RemoteViews applyStandardTemplate = applyStandardTemplate(false, getBigContentViewLayoutResource(min), false);
        applyStandardTemplate.removeAllViews(R.id.media_actions);
        if (min > 0) {
            int i = 0;
            do {
                applyStandardTemplate.addView(R.id.media_actions, generateMediaActionButton(this.mBuilder.A0F.get(i)));
                i++;
            } while (i < min);
        }
        if (this.mShowCancelButton) {
            applyStandardTemplate.setViewVisibility(R.id.cancel_action, 0);
            applyStandardTemplate.setInt(R.id.cancel_action, "setAlpha", this.mBuilder.A0B.getResources().getInteger(R.integer.cancel_button_image_alpha));
    ...
    public void apply(AnonymousClass03Y r3) {
        ...
        A3C.setStyle(mediaStyle);
    }

    public RemoteViews generateContentView() {
        int min;
        RemoteViews applyStandardTemplate = applyStandardTemplate(false, getContentViewLayoutResource(), true);
        int size = this.mBuilder.A0F.size();
        int[] iArr = this.mActionsToShowInCompact;
        if (iArr == null) {
            min = 0;
        } else {
        ...
        if (min > 0) {
            int i = 0;
            while (i < size) {
                applyStandardTemplate.addView(R.id.media_actions, generateMediaActionButton(this.mBuilder.A0F.get(this.mActionsToShowInCompact[i])));
                i++;
                if (i >= min) {
                }
            }
    ...
    public NotificationCompat$MediaStyle(AnonymousClass03h r2) {
        setBuilder(r2);
    }
}
