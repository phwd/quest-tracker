...
public abstract class AbstractC09361bk extends ContentProvider {
    public final AtomicBoolean mInitialized = new AtomicBoolean();
    @Nullable
    public AbstractC02640aw mLogger = null;
    ...
    private void beginTraceSection(String str) {
        if (Systrace.A03(512)) {
            Systrace.A01(512, AnonymousClass006.A07(getClass().getSimpleName(), ".", str));
        }
    }
    ...
    public final int delete(Uri uri, @Nullable String str, String[] strArr) {
        beginTraceSection(LinkedAccountsDumperPlugin.COMMAND_DELETE);
        logUsage(LinkedAccountsDumperPlugin.COMMAND_DELETE);
        try {
            ensureInitializedAndEnforcePermissions();
    ...
    public final Uri insert(Uri uri, ContentValues contentValues) {
        beginTraceSection(LinkedAccountsDumperPlugin.COMMAND_INSERT);
        logUsage(LinkedAccountsDumperPlugin.COMMAND_INSERT);
        try {
            ensureInitializedAndEnforcePermissions();
    ...
    public final int update(Uri uri, ContentValues contentValues, @Nullable String str, @Nullable String[] strArr) {
        beginTraceSection(ConfigUpdaterDumperPlugin.COMMAND_UPDATE);
        logUsage(ConfigUpdaterDumperPlugin.COMMAND_UPDATE);
        try {
            ensureInitializedAndEnforcePermissions();
    ...
    private void logUsage(String str) {
        AbstractC02640aw logger = getLogger();
        if (logger != null) {
            Context context = getContext();
            String formatStrLocaleSafe = StringFormatUtil.formatStrLocaleSafe("%s", getClass().getName());
            try {
                logger.A5U(formatStrLocaleSafe, str, C02870bf.A00(Binder.getCallingUid(), context).toString());
            } catch (Exception unused) {
                logger.A5T(formatStrLocaleSafe, str);
            }
        }
    }
    ...
    public final Cursor query(Uri uri, String[] strArr, String str, String[] strArr2, String str2) {
        beginTraceSection(LinkedAccountsDumperPlugin.COMMAND_QUERY);
        logUsage(LinkedAccountsDumperPlugin.COMMAND_QUERY);
        try {
            ensureInitializedAndEnforceReadOnlyPermissions();
            return doQuery(uri, strArr, str, strArr2, str2);
        } finally {
