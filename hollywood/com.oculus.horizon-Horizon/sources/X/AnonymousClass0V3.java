package X;

import java.io.IOException;

/* renamed from: X.0V3  reason: invalid class name */
public class AnonymousClass0V3 extends AnonymousClass0yl<String> {
    /* JADX DEBUG: Method arguments types fixed to match base method, original types: [X.0zU, java.lang.Object] */
    @Override // X.AnonymousClass0yl
    public final void A03(C09130zU r1, String str) throws IOException {
        r1.A0D(str);
    }

    /* Return type fixed from 'java.lang.Object' to match base method */
    @Override // X.AnonymousClass0yl
    public final String A02(C09120zR r3) throws IOException {
        Integer A0G = r3.A0G();
        if (A0G == AnonymousClass007.A0I) {
            r3.A0P();
            return null;
        } else if (A0G == AnonymousClass007.A0H) {
            return Boolean.toString(r3.A0S());
        } else {
            return r3.A0J();
        }
    }
}
