...
public class NativeCrashDumpReporterUtil {
    ...
    public static boolean processFatMinidump(Context context, CrashReportData crashReportData, Spool.FileBeingConsumed fileBeingConsumed) throws IOException {
        File file = new File(fileBeingConsumed.fileName.getParentFile(), AnonymousClass006.A07(fileBeingConsumed.fileName.getName().split("_")[0], "_", EXTRA_MAP_SUFFIX));
        if (file.exists()) {
    ...
    public static boolean processCoreDump(Context context, CrashReportData crashReportData, Spool.FileBeingConsumed fileBeingConsumed) throws IOException {
        ...
        if (file.exists() && file.length() != 0) {
            if (BuildConfig.CPU_FILTERS.contains("64")) {
            ...
            if (file2.exists() && Math.abs(file.lastModified() - fileBeingConsumed.fileName.lastModified()) <= MobileConfigAppAwareAccessorDecorator.MS_IN_ONE_MINUTE) {
                long currentTimeMillis = System.currentTimeMillis();
                long j = currentTimeMillis - VideoUploaderCleanerServiceManager.STALE_THRESHOLD_MILLIS;
