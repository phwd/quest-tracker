...
public class HttpPostSender implements FlexibleReportSender {
    ...
    private void sendInternal(CrashReportData report) throws IOException {
        ...
        String userId = null;
        if (report.containsKey(ReportField.UID)) {
            userId = (String) report.get(ReportField.UID);
        }
        if (userId != null && !userId.equals("") && !userId.equals("0")) {
            extraHeaders.put(HttpHeaders.COOKIE, "c_user=" + userId);
        }
    ...
    public void setUseMultipartPost(boolean multipart) {
        this.mUseMultipartPost = multipart;
    }

    public void setUseZstd(boolean useZstd) {
