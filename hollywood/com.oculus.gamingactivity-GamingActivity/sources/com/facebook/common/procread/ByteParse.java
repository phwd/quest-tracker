...
public final class ByteParse {
    ...
    private static int maxExponent = FrameMetricsAggregator.EVERY_DURATION;
    private static double[] powersOf10 = {10.0d, 100.0d, 10000.0d, 1.0E8d, 1.0E16d, 1.0E32d, 1.0E64d, 1.0E128d, 1.0E256d};

    private ByteParse() {
    }

    public static float strtof(byte[] string, int start, @Nullable StrofResult outRes) {
        boolean sign;
        int fracExp;
        int pPtr;
        int exp;
    ...
    public static long strtoll(byte[] string, int start, int givenBase, @Nullable StrtollResult outRes) {
        byte c;
        byte c2;
        ...
        long cutoff = neg ? Clock.MAX_TIME : Clock.MAX_TIME;
        ...
        if (any < 0) {
            acc = neg ? Long.MIN_VALUE : Clock.MAX_TIME;
