...
public final class ConnectionPool {
    ...
    private int pruneAndGetAllocationCount(RealConnection realConnection, long j) {
        ...
        while (i < list.size()) {
            if (list.get(i).get() != null) {
                ...
                logger.warning("A connection to " + realConnection.getRoute().address.url + " was leaked. Did you forget to close a response body?");
                list.remove(i);
    ...
    public ConnectionPool(int i, long j, TimeUnit timeUnit) {
        ...
        if (j <= 0) {
            throw new IllegalArgumentException(AnonymousClass006.A04("keepAliveDuration <= 0: ", j));
        }
    }
}
