...
public class CallerInfoHelper {
    ...
    public static CallerInfo getCallerInfo(Intent launchingIntent, boolean enforceTimeout) {
        ...
        if (launchingIntent.hasExtra(CALLER_INFO) && (pendingIntent = (PendingIntent) launchingIntent.getParcelableExtra(CALLER_INFO)) != null) {
            if (isOnOrAboveApi17()) {
                str = Api17Utils.getCreatorPackage(pendingIntent);
            } else {
                str = pendingIntent.getTargetPackage();
            }
            callerPackageName = str;
            if (isOnOrAboveApi17()) {
            ...
            if (metaData != null) {
                try {
                    JSONObject metaDataJson = new JSONObject(new String(Base64.decode(metaData, (int) BASE64_FLAG), ENCODING));
    ...
    static String encodeCallerInfoMetaData(String callerDomain, long creationTime) {
        ...
        try {
            ...
            try {
                return Base64.encodeToString(json.toString().getBytes(ENCODING), BASE64_FLAG);
