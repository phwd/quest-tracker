...
public class PatternStringUtils {
    ...
    /* JADX INFO: Multiple debug info for r2v2 int: [D('groupingSize' int), D('groupingLength' int)] */
    /* JADX INFO: Multiple debug info for r6v12 int: [D('digitsStringScale' int), D('di' int)] */
    public static String propertiesToPatternString(DecimalFormatProperties properties) {
        ...
        int m0 = Math.max(groupingLength, digitsString.length() + digitsStringScale2);
        int m02 = maxInt != 100 ? Math.max(maxInt, m0) - 1 : m0 - 1;
        int mN = maxFrac != 100 ? Math.min(-maxFrac, digitsStringScale2) : digitsStringScale2;
        int magnitude = m02;
        while (magnitude >= mN) {
            int di = ((digitsString.length() + digitsStringScale2) - magnitude) - 1;
            if (di >= 0) {
                minInt = minInt2;
