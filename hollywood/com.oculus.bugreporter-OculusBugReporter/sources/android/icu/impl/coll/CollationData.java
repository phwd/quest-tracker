...
public final class CollationData {
    ...
    /* JADX INFO: Multiple debug info for r14v9 int: [D('i' int), D('script' int)] */
        ...
        if (length != 0) {
            ...
            if (length != 1 || reorder[0] != 103) {
                ...
                while (true) {
                    ...
                    int i3 = script + 1;
                    int script2 = reorder[script];
                    if (script2 == index) {
                        while (i3 < length) {
                            length--;
                            int script3 = reorder[length];
                            if (script3 == index) {
                                throw new IllegalArgumentException("setReorderCodes(): duplicate UScript.UNKNOWN");
                            } else if (script3 != -1) {
                                int index2 = getScriptIndex(script3);
                                if (index2 == 0) {
                                    index = 103;
                                    ...
                                    throw new IllegalArgumentException("setReorderCodes(): duplicate or equivalent script " + scriptCodeString(script3));
                                }
                            } else {
                        ...
                        int index3 = getScriptIndex(script2);
                        if (index3 == 0) {
                            script = i3;
                            index = 103;
                ...
                while (true) {
                    ...
                    if (table[i4] == 0) {
                        ...
                        lowStart3 = addLowScriptRange(table, i4, lowStart4 == 1 ? 1 : 0);
                    }
                    i4++;
                    lowStart3 = lowStart3;
                }
                if (lowStart3 <= highLimit) {
                    ...
                    while (true) {
                        ...
                        while (true) {
                            ...
                            if (i5 >= cArr3.length - 1) {
                                break;
                            }
                            short s = table[i5];
                            if (s != 255) {
                                nextOffset = s - (cArr3[i5] >> '\b');
                        ...
                        if (offset != 0 || i5 < this.scriptStarts.length - 1) {
                            ranges.addElement((this.scriptStarts[i5] << 16) | (65535 & offset));
                        }
                        if (i5 != this.scriptStarts.length - 1) {
                            offset = nextOffset;
                            i5++;
                        } else {
                            return;
    ...
    private int addLowScriptRange(short[] table, int index, int lowStart) {
        ...
        return ((lowStart & MotionEventCompat.ACTION_POINTER_INDEX_MASK) + ((c2 & 65280) - (65280 & c))) | (c2 & 255);
    }

    private int addHighScriptRange(short[] table, int index, int highLimit) {
        char c = this.scriptStarts[index + 1];
        if ((c & 255) > (highLimit & 255)) {
            highLimit += InputDeviceCompat.SOURCE_ANY;
        }
        char c2 = this.scriptStarts[index];
        int highLimit2 = ((highLimit & MotionEventCompat.ACTION_POINTER_INDEX_MASK) - ((c & 65280) - (65280 & c2))) | (c2 & 255);
