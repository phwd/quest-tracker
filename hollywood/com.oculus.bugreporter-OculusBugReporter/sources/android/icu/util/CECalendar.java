...
abstract class CECalendar extends Calendar {
    ...
    public static int ceToJD(long year, int month, int day, int jdEpochOffset) {
        ...
        return (int) (((((((long) jdEpochOffset) + (365 * year2)) + floorDivide(year2, 4)) + ((long) (month2 * 30))) + ((long) day)) - 1);
    }

    public static void jdToCE(int julianDay, int jdEpochOffset, int[] fields) {
        int[] r4 = new int[1];
        int doy = 365;
        fields[0] = (floorDivide(julianDay - jdEpochOffset, 1461, r4) * 4) + ((r4[0] / 365) - (r4[0] / 1460));
        if (r4[0] != 1460) {
            doy = r4[0] % 365;
        }
        fields[1] = doy / 30;
