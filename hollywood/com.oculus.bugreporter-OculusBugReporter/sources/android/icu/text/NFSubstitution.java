            ...
            case '<':
                if (rule.getBaseValue() != -1) {
                    if (rule.getBaseValue() != -2) {
                        if (rule.getBaseValue() != -3) {
                            if (rule.getBaseValue() != -4) {
                                ...
                                return new MultiplierSubstitution(pos2, rule, ruleSet2, description);
                            }
                        }
                    }
                    return new IntegralPartSubstitution(pos2, ruleSet2, description);
                }
                throw new IllegalArgumentException("<< not allowed in negative-number rule");
            case '=':
                return new SameValueSubstitution(pos2, ruleSet2, description);
            case '>':
                if (rule.getBaseValue() == -1) {
                    return new AbsoluteValueSubstitution(pos2, ruleSet2, description);
                }
                if (rule.getBaseValue() == -2 || rule.getBaseValue() == -3 || rule.getBaseValue() == -4) {
                    return new FractionalPartSubstitution(pos2, ruleSet2, description);
                }
                if (!ruleSet2.isFractionSet()) {
