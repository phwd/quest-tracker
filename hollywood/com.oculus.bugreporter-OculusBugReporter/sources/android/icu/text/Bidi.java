...
public class Bidi {
    ...
    /* JADX INFO: Multiple debug info for r4v5 short: [D('nextStrongProp' short), D('resProp' short)] */
    private void resolveImplicitLevels(int start, int limit, short sor, short eor) {
        ...
        while (true) {
            ...
            if (actionImp != 0) {
                ...
                short resProp = impTabProps[stateImp][15];
    ...
    /* JADX INFO: Multiple debug info for r5v9 int: [D('index' int), D('runLength' int)] */
    /* access modifiers changed from: package-private */
    public void setParaRunsOnly(char[] parmText, byte parmParaLevel) {
        ...
        while (i5 >= 0) {
            ...
            if (runLength < 2) {
                ...
                int j2 = start;
                while (j2 != limit) {
                    int runLength3 = visualMap[j2];
                    int index12 = visualMap[j2 + step];
                    if (Bidi_Abs(runLength3 - index12) == 1 && saveLevels[runLength3] == saveLevels[index12]) {
                        saveDirection2 = saveDirection3;
                    } else {
                        int logicalPos2 = Bidi_Min(visualMap[start], runLength3);
