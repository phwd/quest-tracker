...
public final class UScript {
    ...
    private static int[] getCodesFromLocale(ULocale locale) {
        ...
        if (script.length() == 0 || (scriptCode = getCodeFromName(script)) == -1) {
            return null;
        }
    ...
    public static final int[] getCode(String nameOrAbbrOrLocale) {
        ...
        if (nameOrAbbrOrLocale.indexOf(95) < 0 && nameOrAbbrOrLocale.indexOf(45) < 0) {
            int propNum2 = UCharacter.getPropertyValueEnumNoThrow(UProperty.SCRIPT, nameOrAbbrOrLocale);
            if (propNum2 != -1) {
        ...
        if (triedCode || (propNum = UCharacter.getPropertyValueEnumNoThrow(UProperty.SCRIPT, nameOrAbbrOrLocale)) == -1) {
            return null;
        }
    ...
    public static final boolean hasScript(int c, int sc) {
        ...
        if (scriptX >= 4194304) {
            ...
            int scx2 = scx;
            if (scriptX >= 12582912) {
                scx2 = scriptExtensions[scx + 1];
            }
            int scx3 = scx2;
            if (sc > 32767) {
                return false;
            }
            while (sc > scriptExtensions[scx3 == 1 ? 1 : 0]) {
                scx3 = (scx3 == 1 ? 1 : 0) + 1;
            }
            if (sc == (32767 & scriptExtensions[scx3])) {
                return true;
            }
            return false;
        } else if (sc == scriptX) {
    ...
    /* JADX INFO: Multiple debug info for r2v3 char: [D('scx' int), D('sx' int)] */
    ...
    public static final class ScriptMetadata {
        ...
        /* access modifiers changed from: private */
        public static final int getScriptProps(int script) {
            if (script < 0) {
                return 0;
            }
            int[] iArr = SCRIPT_PROPS;
            if (script < iArr.length) {
                return iArr[script];
            }
            return 0;
        }
