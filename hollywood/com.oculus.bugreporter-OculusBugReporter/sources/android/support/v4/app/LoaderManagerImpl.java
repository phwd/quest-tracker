...
public class LoaderManagerImpl extends LoaderManager {
    ...
    public static class LoaderObserver<D> implements Observer<D> {
        ...
        public void onChanged(@Nullable D data) {
            if (LoaderManagerImpl.DEBUG) {
                Log.v(LoaderManagerImpl.TAG, "  onLoadFinished in " + ((Object) this.mLoader) + PluralRules.KEYWORD_RULE_SEPARATOR + this.mLoader.dataToString(data));
            }
            ...
            if (loader != null) {
                if (loader.getClass().isMemberClass()) {
                    if (!Modifier.isStatic(loader.getClass().getModifiers())) {
                        throw new IllegalArgumentException("Object returned from onCreateLoader must not be a non-static inner member class: " + ((Object) loader));
                    }
                }
                LoaderInfo<D> info = new LoaderInfo<>(id, args, loader, priorLoader);
                try {
                    if (DEBUG) {
                        Log.v(TAG, "  Created new loader " + ((Object) info));
                    }
    ...
    public <D> Loader<D> initLoader(int id, @Nullable Bundle args, @NonNull LoaderManager.LoaderCallbacks<D> callback) {
        if (this.mLoaderViewModel.isCreatingLoader()) {
            ...
            if (DEBUG) {
                Log.v(TAG, "initLoader in " + ((Object) this) + ": args=" + ((Object) args));
            }
            if (info == null) {
                return createAndInstallLoader(id, args, callback, null);
            }
            if (DEBUG) {
                Log.v(TAG, "  Re-using existing loader " + ((Object) info));
            }
            return info.setCallback(this.mLifecycleOwner, callback);
        } else {
    ...
    public <D> Loader<D> restartLoader(int id, @Nullable Bundle args, @NonNull LoaderManager.LoaderCallbacks<D> callback) {
        if (this.mLoaderViewModel.isCreatingLoader()) {
            ...
            if (DEBUG) {
                Log.v(TAG, "restartLoader in " + ((Object) this) + ": args=" + ((Object) args));
            }
            LoaderInfo<D> info = this.mLoaderViewModel.getLoader(id);
    ...
    public void destroyLoader(int id) {
        if (this.mLoaderViewModel.isCreatingLoader()) {
            ...
            if (DEBUG) {
                Log.v(TAG, "destroyLoader in " + ((Object) this) + " of " + id);
            }
