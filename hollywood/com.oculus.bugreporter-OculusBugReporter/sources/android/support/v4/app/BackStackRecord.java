    ...
    public void dump(String prefix, PrintWriter writer, boolean full) {
        ...
        if (!this.mOps.isEmpty()) {
            ...
            for (int opNum = 0; opNum < numOps; opNum++) {
                ...
                writer.print(cmdStr);
                writer.print(Padder.FALLBACK_PADDING_STRING);
    ...
    private void doAddOp(int containerViewId, Fragment fragment, @Nullable String tag, int opcmd) {
        ...
        if (tag != null) {
            if (fragment.mTag == null || tag.equals(fragment.mTag)) {
                ...
                throw new IllegalStateException("Can't change tag of fragment " + ((Object) fragment) + ": was " + fragment.mTag + " now " + tag);
            }
        }
        if (containerViewId != 0) {
            if (containerViewId == -1) {
                throw new IllegalArgumentException("Can't add fragment " + ((Object) fragment) + " with tag " + tag + " to container view with no id");
            } else if (fragment.mFragmentId == 0 || fragment.mFragmentId == containerViewId) {
                fragment.mFragmentId = containerViewId;
                fragment.mContainerId = containerViewId;
            } else {
                throw new IllegalStateException("Can't change container ID of fragment " + ((Object) fragment) + ": was " + fragment.mFragmentId + " now " + containerViewId);
            }
        }
    ...
    public void bumpBackStackNesting(int amt) {
        if (this.mAddToBackStack) {
            if (FragmentManagerImpl.DEBUG) {
                Log.v(TAG, "Bump nesting in " + ((Object) this) + " by " + amt);
            }
            int numOps = this.mOps.size();
            for (int opNum = 0; opNum < numOps; opNum++) {
                Op op = this.mOps.get(opNum);
                if (op.fragment != null) {
                    ...
                    if (FragmentManagerImpl.DEBUG) {
                        Log.v(TAG, "Bump nesting of " + ((Object) op.fragment) + " to " + op.fragment.mBackStackNesting);
                    }
                }
            }
        }
