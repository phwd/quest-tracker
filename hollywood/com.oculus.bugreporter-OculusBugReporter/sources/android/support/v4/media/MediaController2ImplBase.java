...
public class MediaController2ImplBase implements MediaController2.SupportLibraryImpl {
    ...
    private void connectToService() {
        ...
        synchronized (this.mLock) {
            if (!this.mContext.bindService(intent, this.mServiceConnection, 1)) {
                Log.w(TAG, "bind to " + ((Object) this.mToken) + " failed");
            } else if (DEBUG) {
                Log.d(TAG, "bind to " + ((Object) this.mToken) + " success");
            }
        }
    }
    ...
    public IMediaSession2 getSessionInterfaceIfAble(SessionCommand2 command) {
        synchronized (this.mLock) {
            if (!this.mAllowedCommands.hasCommand(command)) {
                Log.w(TAG, "Controller isn't allowed to call command, command=" + ((Object) command));
                return null;
            }
            return this.mISession2;
        }
    ...
    public void onCustomCommand(final SessionCommand2 command, final Bundle args, final ResultReceiver receiver) {
        if (DEBUG) {
            Log.d(TAG, "onCustomCommand cmd=" + ((Object) command));
        }
        this.mCallbackExecutor.execute(new Runnable() {
    ...
    public class SessionServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            if (MediaController2ImplBase.DEBUG) {
                Log.d(MediaController2ImplBase.TAG, "onServiceConnected " + ((Object) name) + Padder.FALLBACK_PADDING_STRING + ((Object) this));
            }
            if (!MediaController2ImplBase.this.mToken.getPackageName().equals(name.getPackageName())) {
                Log.wtf(MediaController2ImplBase.TAG, ((Object) name) + " was connected, but expected pkg=" + MediaController2ImplBase.this.mToken.getPackageName() + " with id=" + MediaController2ImplBase.this.mToken.getId());
                return;
            }
        ...
        public void onServiceDisconnected(ComponentName name) {
            if (MediaController2ImplBase.DEBUG) {
                Log.w(MediaController2ImplBase.TAG, "Session service " + ((Object) name) + " is disconnected.");
            }
        }
