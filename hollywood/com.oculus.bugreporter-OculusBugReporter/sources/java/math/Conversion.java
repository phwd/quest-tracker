...
public class Conversion {
    ...
    /* JADX INFO: Multiple debug info for r13v6 int: [D('bigRadix' int), D('tempLen' int)] */
    static String bigInteger2String(BigInteger val, int radix) {
        ...
        if (numberLength == 1) {
            ...
            if (radix != 16) {
                ...
                int tempLen = bigRadices[radix - 2];
                currentChar = resLengthInChars;
                int tempLen2 = numberLength;
                while (true) {
                    int resDigit = Division.divideArrayByInt(temp, temp, tempLen2, tempLen);
                    while (true) {
                        currentChar--;
