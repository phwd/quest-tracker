...
public class MethodHandles {
    ...
    public static <T extends Member> T reflectAs(Class<T> expected, MethodHandle target) {
        return expected.cast(getMethodHandleImpl(target).getMemberInternal());
    }
    ...
    public static final class Lookup {
        ...
        private static void checkUnprivilegedlookupClass(Class<?> lookupClass2, int allowedModes2) {
            ...
            if (name.startsWith("java.lang.invoke.")) {
                throw MethodHandleStatics.newIllegalArgumentException("illegal lookupClass: " + ((Object) lookupClass2));
            ...
            if (method == null) {
                try {
                    ...
                    if (Modifier.isStatic(m.getModifiers())) {
                        throw new IllegalAccessException("Method" + ((Object) m) + " is static");
                    }
                } catch (NoSuchMethodException e) {
                }
                throw new NoSuchMethodException(name + Padder.FALLBACK_PADDING_STRING + Arrays.toString(type.ptypes()));
            }
            checkReturnType(method, type);
        ...
        private MethodHandle findAccessor(Field field, Class<?> refc, Class<?> type, int kind, boolean performAccessChecks) throws IllegalAccessException {
            ...
            boolean isSetterKind = kind == 10 || kind == 12;
            commonFieldChecks(field, refc, type, kind == 11 || kind == 12, performAccessChecks);
        ...
        public MethodHandle unreflectSpecial(Method m, Class<?> specialCaller) throws IllegalAccessException {
            if (m == null) {
                ...
                return findSpecial(m, MethodType.methodType(m.getReturnType(), m.getParameterTypes()), m.getDeclaringClass(), specialCaller);
            } else {
                throw new NullPointerException("specialCaller == null");
            }
        }
        ...
        public VarHandle unreflectVarHandle(Field f) throws IllegalAccessException {
            commonFieldChecks(f, f.getDeclaringClass(), f.getType(), Modifier.isStatic(f.getModifiers()), true);
        ...
        private void throwMakeAccessException(String message, Object from) throws IllegalAccessException {
            String message2 = message + PluralRules.KEYWORD_RULE_SEPARATOR + toString();
            if (from != null) {
    ...
    private static void checkClassIsArray(Class<?> c) {
        if (!c.isArray()) {
            throw new IllegalArgumentException("Not an array type: " + ((Object) c));
        }
    }
    ...
    public static MethodHandle foldArguments(MethodHandle target, MethodHandle combiner) {
        foldArgumentChecks(0, target.type(), combiner.type());
    ...
    static RuntimeException misMatchedTypes(String what, MethodType t1, MethodType t2) {
        return MethodHandleStatics.newIllegalArgumentException(what + " must match: " + ((Object) t1) + " != " + ((Object) t2));
    }
    ...
    public static MethodHandle catchException(MethodHandle target, Class<? extends Throwable> exType, MethodHandle handler) {
        ...
        if (htype.parameterCount() < 1 || !htype.parameterType(0).isAssignableFrom(exType)) {
            ...
            if (targs.equals(hargs2) || ((hpc = hargs2.size()) < targs.size() && targs.subList(0, hpc).equals(hargs2))) {
