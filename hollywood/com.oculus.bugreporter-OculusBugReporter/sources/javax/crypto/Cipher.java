...
public class Cipher {
    ...
    private static String[] tokenizeTransformation(String transformation2) throws NoSuchAlgorithmException {
        ...
        if (count == 0 || count == 2 || parser.hasMoreTokens()) {
            throw new NoSuchAlgorithmException("Invalid transformation format:" + transformation2);
        } else if (parts[0] != null && parts[0].length() != 0) {
            return parts;
        } else {
    ...
    static CipherSpiAndProvider tryCombinations(InitParams initParams, Provider provider2, String[] tokenizedTransformation2) throws InvalidKeyException, InvalidAlgorithmParameterException {
        ...
        if (cause instanceof InvalidKeyException) {
            ...
            throw new InvalidKeyException("No provider offers " + Arrays.toString(tokenizedTransformation2) + " for " + initParams.key.getAlgorithm() + " key of class " + initParams.key.getClass().getName() + " and export format " + initParams.key.getFormat());
        }
    }
    ...
    public static /* synthetic */ class AnonymousClass1 {
        ...
        static {
            try {
                $SwitchMap$javax$crypto$Cipher$InitType[InitType.ALGORITHM_PARAMS.ordinal()] = 1;
