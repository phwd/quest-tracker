...
public final class MultipartBuilder {
    ...
    public static final class MultipartRequestBody extends RequestBody {
        ...
        public void writeTo(Du du) throws IOException {
            writeOrCountBytes(du, false);
        }

        /* JADX DEBUG: Multi-variable search result rejected for r4v0, resolved type: X.8k */
        /* JADX DEBUG: Multi-variable search result rejected for r4v1, resolved type: X.8k */
        /* JADX DEBUG: Multi-variable search result rejected for r4v2, resolved type: X.8k */
        /* JADX WARN: Multi-variable type inference failed */
        private long writeOrCountBytes(Du du, boolean z) throws IOException {
            AnonymousClass8k r4;
            if (z) {
                du = new AnonymousClass8k();
                r4 = du;
            } else {
                r4 = 0;
            }
            ...
            for (int i = 0; i < size; i++) {
                ...
                du.A3v(MultipartBuilder.DASHDASH);
                du.A3u(this.boundary);
                byte[] bArr = MultipartBuilder.CRLF;
                du.A3v(bArr);
                if (headers != null) {
                    int length = headers.namesAndValues.length >> 1;
                    for (int i2 = 0; i2 < length; i2++) {
                        du.A44(headers.name(i2));
                        du.A3v(MultipartBuilder.COLONSPACE);
                        du.A44(headers.value(i2));
                        du.A3v(bArr);
                    }
                }
                ...
                if (contentType2 != null) {
                    du.A44("Content-Type: ");
                    du.A44(contentType2.toString());
                    du.A3v(bArr);
                }
                long contentLength2 = requestBody.contentLength();
                if (contentLength2 != -1) {
                    du.A44("Content-Length: ");
                    du.A3z(contentLength2);
                    du.A3v(MultipartBuilder.CRLF);
                } else if (z) {
                    r4.A08();
                    return -1;
                }
                ...
                if (z) {
                    ...
                    this.partBodies.get(i).writeTo(du);
                }
                du.A3v(bArr2);
            }
            du.A3v(MultipartBuilder.DASHDASH);
        ...
        public MultipartRequestBody(MediaType mediaType, WM wm, List<Headers> list, List<RequestBody> list2) {
            if (mediaType != null) {
                this.boundary = wm;
