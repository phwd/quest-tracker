...
public abstract class AbstractC0195ed extends ContentProvider {
    public final AtomicBoolean mInitialized = new AtomicBoolean();
    @Nullable
    public AbstractC0199er mLogger = null;
    ...
    private void beginTraceSection(String str) {
        if (Systrace.A03(512)) {
            Systrace.A01(512, AnonymousClass06.A04(getClass().getSimpleName(), ".", str));
        }
    }
    ...
    private void logUsage(String str) {
        AbstractC0199er logger = getLogger();
        if (logger != null) {
            Context context = getContext();
            String formatStrLocaleSafe = StringFormatUtil.formatStrLocaleSafe("%s", getClass().getName());
            try {
                int callingUid = Binder.getCallingUid();
                String[] A06 = C0214fc.A06(context, callingUid);
                logger.A2M(formatStrLocaleSafe, str, new fX(callingUid, Collections.unmodifiableList(Arrays.asList(A06)), C0214fc.A04(context, A06), null, null).toString());
            } catch (Exception unused) {
                logger.A2L(formatStrLocaleSafe, str);
            }
        }
    }
