...
@DoNotOptimize
public final class ErrorReporter implements JC {
    ...
    public static class AcraReportHandler implements ReportHandler {
        ...
        public boolean handleReport(ErrorReporter errorReporter, Spool.FileBeingConsumed fileBeingConsumed, String str, boolean z) {
            ...
            try {
                ...
                Mu.A08("ACRA", e, "Failed to send crash reports");
                ErrorReporter.deleteFile(file);
                return false;
            } catch (IOException e2) {
                Mu.A0B("ACRA", e2, "Failed to load crash report for %s", name);
                ErrorReporter.deleteFile(file);
                return false;
            } catch (ReportSenderException e3) {
                Mu.A0B("ACRA", e3, "Failed to send crash report for %s", name);
                return false;
            }
        }
    ...
    private void buildAttachmentWrapperCrashReport(CrashReportData crashReportData, CrashReportType crashReportType, @Nullable Spool.FileBeingConsumed fileBeingConsumed, @Nullable Writer writer, boolean z) {
        try {
            ...
            put(ReportField.REPORT_LOAD_THROW, AnonymousClass06.A04("retrieve exception: ", th.getMessage()), crashReportData, writer);
        }
    }
    ...
    public static boolean deleteFile(File file) {
        ...
        file.getName();
        if (!delete) {
            Mu.A06("ACRA", "Could not delete error report: %s", file.getName());
        }
        return delete;
    }
    ...
    private void uncaughtExceptionImpl(Thread thread, Throwable th, boolean z) throws Throwable {
        ...
        int i = 3;
        try {
            Mu.A05("ACRA", "ACRA caught a %s exception for %s version %s. Building report.", th.getClass().getSimpleName(), this.mContext.getPackageName(), this.mAppVersionCode);
        } catch (Throwable th3) {
        ...
        if (translateException != null) {
            ...
            try {
                ...
                if (!z) {
                    Mu.A08("ACRA", th4, "error during error reporting: will attempt to report error");
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:9:0x001b, code lost:
        r3 = new java.io.FileOutputStream(new java.io.File(X.AnonymousClass06.A04(r1, com.facebook.acra.ErrorReporter.ANR_EXTRA_PROPERTIES_EXTENSION)), true);
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    try {
                        Mu.A08("ACRA", e, "GLC getNewLibs IO Exception");
                        safeClose(closeable);
    ...
    private void checkNativeReportsOnApplicationStart() {
        ...
        if (maxPendingMiniDumpReports < Integer.MAX_VALUE && roughlyCountPendingReportsOfType > maxPendingMiniDumpReports) {
            Mu.A06(TAG, "Minidump count above threshold %d", Integer.valueOf(roughlyCountPendingReportsOfType));
    ...
    private File createBackUpDirectory(CrashReportType crashReportType) {
        ...
        try {
            ...
            try {
                ...
                e = e2;
                Mu.A0B("ACRA", e, "Failed to create backup directory %s", REPORTED_CRASH_DIR);
                return file;
            }
    ...
    private CrashReportData createCrashReportFromStackTrace(String str, CrashReportType crashReportType) {
        ...
        try {
            ...
            e.printStackTrace();
            crashReportData.put(ReportField.REPORT_LOAD_THROW, AnonymousClass06.A04("throwable: ", e.getMessage()));
            Mu.A08("ACRA", e, "Could not load crash report. File will be deleted.");
        }
    ...
    public static String getLogcatFileName(Spool.FileBeingConsumed fileBeingConsumed, CrashReportType crashReportType) {
        ...
        try {
            ...
            Mu.A0B("ACRA", e, "Failed to find logcat file %s", fileBeingConsumed.fileName);
            return null;
        }
    }
    ...
    private void maybeRemoveAnrReports() {
        ...
        if (maxPendingAnrReports < Integer.MAX_VALUE && roughlyCountPendingReportsOfType > maxPendingAnrReports) {
            Mu.A06(TAG, "Anr count %d above threshold %d", Integer.valueOf(roughlyCountPendingReportsOfType), Integer.valueOf(maxPendingAnrReports));
    ...
    private void removeCrashFiles(final CrashReportType crashReportType, int i) {
        if (i > 0) {
            try {
                ...
                if (snapshot == null) {
                    Mu.A01(TAG, "removeCrashFiles no snapshot");
                    return;
                }
                    ...
                    if (next.fileName != null) {
                        ...
                        if (strArr != null) {
                            for (String str : strArr) {
                                new File(AnonymousClass06.A04(next.fileName.getCanonicalPath(), str)).delete();
                            }
                        }
                        if (!next.fileName.delete()) {
                            Mu.A06(TAG, "removeCrashFiles Crash file not deleted %s", next.fileName.getAbsolutePath());
                        } else {
                            i--;
                        }
                    }
                ...
                Mu.A08(TAG, th, "removeCrashFiles");
            }
        }
    }
        ...
        if (!arrayList.isEmpty()) {
            ...
            while (it.hasNext()) {
                ...
                try {
                    ...
                    if (z) {
                        Mu.A0C("ACRA", e, "ReportSender of class %s failed but other senders completed their task. ACRA will not send this report again.", reportSender.getClass().getName());
    ...
    private void slurpAttachment(CrashReportData crashReportData, InputStream inputStream, CrashReportType crashReportType, long j) throws IOException {
        if (crashReportType == CrashReportType.NATIVE_CRASH_REPORT) {
            try {
                ...
                Mu.A09(TAG, e, "error attaching activity information");
            }
            try {
                attachIabInfo(crashReportData);
            } catch (IOException e2) {
                Mu.A09(TAG, e2, "error attaching IAB information");
            }
        }
    ...
    private boolean writeJsonReport(CrashReportData crashReportData) {
        ...
        File file = new File(this.mContext.getDir(ErrorReportingConstants.TRACE_UPLOAD_DIR, 0), AnonymousClass06.A04(str, ".gz"));
        HashMap hashMap = new HashMap();
        for (Map.Entry<String, InputStreamField> entry : crashReportData.mInputStreamFields.entrySet()) {
    ...
    private void writeToLogBridge(String str, String str2, Throwable th, @Nullable String str3) {
        ...
        for (StackTraceElement stackTraceElement2 : stackTrace) {
            if (stackTraceElement2.getClassName().equals(stackTraceElement.getClassName()) && stackTraceElement2.getMethodName().equals(stackTraceElement.getMethodName())) {
                Mu.A08(TAG, th, "Unable to log over log bridge due to exception.");
                return;
            }
        }
        ...
        if (logBridge != null) {
            if (str3 != null) {
                str2 = AnonymousClass06.A05(str2, "\n", str3);
                th = null;
            }
    ...
    public boolean addToAnrInProgressUpdateFile(Map<String, String> map) throws IOException {
        synchronized (this.mAnrFilesInProgress) {
            ...
            FileOutputStream fileOutputStream = new FileOutputStream(new File(AnonymousClass06.A04(this.mAnrFilesInProgress.iterator().next(), ANR_EXTRA_PROPERTIES_EXTENSION)), true);
    ...
    public String getLogcatOutputIfPidFound(boolean z, @Nullable Integer num) {
        ...
        if (collectLogCat == null || (num != null && !Pattern.compile(AnonymousClass06.A05("^\\d+-\\d+\\s+\\d+:\\d+:\\d+\\.\\d+\\s+", num.toString(), "\\s+\\d+\\s+[A-Z]\\s+(.*?)$"), 8).matcher(collectLogCat).find())) {
            return null;
        }
    ...
    @Override // X.JC
    public void handleUncaughtException(Thread thread, Throwable th, @Nullable J8 j8) {
        ...
        Mu.A0B(TAG, th, "Maximum pending Java crash threshold reached, Not storing count=%d", Integer.valueOf(roughlyCountPendingReportsOfType));
    }
        ...
        if (!this.mInitializationComplete) {
            ...
            if (this.mContext != null) {
                ...
                if (this.mInstallTime == 0) {
                    Mu.A01("ACRA", "could not retrieve APK mod time");
                }
    ...
    public void reportErrorAndTerminate(Thread thread, Throwable th) {
        synchronized (JB.class) {
            JB.A00().uncaughtException(thread, th);
    ...
    public int roughlyCountPendingReportsOfType(CrashReportType... crashReportTypeArr) {
        if (this.mContext == null) {
            Mu.A00("ACRA", "Trying to get ACRA reports but ACRA is not initialized.");
            return 0;
        }
        int i = 0;
        for (CrashReportType crashReportType : crashReportTypeArr) {
    ...
    public void updateGLCwithSystemLibs(Spool.FileBeingConsumed fileBeingConsumed) {
        ...
        if (!crashDumpSysLibPath.exists()) {
            try {
                ...
                Mu.A08("ACRA", e, "Failed to create GLC Lib file");
                return;
            }
        }
        try {
            ...
            Mu.A08("ACRA", e2, "Failed to create GLC Lib file");
        }
    }
        ...
        if (hashSet != null && !hashSet.isEmpty()) {
            ...
            try {
                ...
                try {
                    ...
                    while (it.hasNext()) {
                        bufferedWriter.write(AnonymousClass06.A04(it.next(), "\n"));
                    }
                    ...
                    try {
                        Mu.A08("ACRA", e, "GLC file to write Exception");
                        safeClose(closeable);
    ...
    private String genCrashReportFileName(Class cls, UUID uuid, String str) {
        ...
        if (this.mAppVersionCode != null) {
            str2 = AnonymousClass06.A04("-", this.mAppVersionCode);
        } else {
            str2 = "";
        }
        return AnonymousClass06.A07(obj, "-", simpleName, str2, str);
    }
    ...
    private void populateConstantFields() {
        ...
        String str4;
        int A00 = IK.A00();
        this.mAppVersionCode = Integer.toString(IK.A01());
        ...
        if (this.mConfig.shouldReportField(ReportField.ANDROID_ID)) {
            try {
                ...
                Mu.A02(TAG, "Failed to fetch the constant field ANDROID_ID", e);
                str4 = "unknown";
            }
        ...
        if (this.mConfig.shouldReportField(ReportField.IS_RELABELED)) {
            treeMap.put(ReportField.IS_RELABELED, String.valueOf(IK.A02()));
        }
    ...
    private void tryLogInternalError(@Nullable String str, Throwable th) {
        ...
        try {
            Mu.A0B("ACRA", th, "internal ACRA error: %s: ", str);
        } catch (Throwable unused) {
        }
    }
