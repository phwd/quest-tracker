...
public final class Http2Reader implements Closeable {
    ...
    public boolean nextFrame(boolean requireSettings, Handler handler) throws IOException {
        try {
            ...
            byte type = (byte) (this.source.readByte() & UnsignedBytes.MAX_VALUE);
            if (!requireSettings || type == 4) {
                byte flags = (byte) (this.source.readByte() & UnsignedBytes.MAX_VALUE);
    ...
    private void readHeaders(Handler handler, int length, byte flags, int streamId) throws IOException {
        ...
        if ((flags & 8) != 0) {
            padding = (short) (this.source.readByte() & UnsignedBytes.MAX_VALUE);
        }
        if ((flags & 32) != 0) {
    ...
    private void readPriority(Handler handler, int streamId) throws IOException {
        ...
        handler.priority(streamId, w1 & Integer.MAX_VALUE, (this.source.readByte() & UnsignedBytes.MAX_VALUE) + 1, (Integer.MIN_VALUE & w1) != 0);
    }
    ...
    static int readMedium(BufferedSource source2) throws IOException {
        return ((source2.readByte() & UnsignedBytes.MAX_VALUE) << 16) | ((source2.readByte() & UnsignedBytes.MAX_VALUE) << 8) | (source2.readByte() & UnsignedBytes.MAX_VALUE);
    }
