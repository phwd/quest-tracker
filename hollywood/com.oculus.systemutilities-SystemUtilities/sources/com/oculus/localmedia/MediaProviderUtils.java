...
public class MediaProviderUtils {
    ...
    private static final ArrayList<ExifChecks> exifList = new ArrayList<>(Arrays.asList(new ExifChecks("Model", "RICOH THETA"), new ExifChecks("Orientation", "equirectangular"), new ExifChecks("Software", "photosynth")));
    ...
    public static boolean isPrivateFolder(String path) {
        if (path == null || !new File(path + File.separator + ".nomedia").exists()) {
            return false;
        }
        return true;
    }
    ...
    public static ArrayList<String> getHiddenFolders(Context context) {
        ...
        Cursor cursor = context.getContentResolver().query(MediaStore.Files.getContentUri("external"), new String[]{"_data"}, "media_type=0" + " AND " + "title" + " LIKE ?", new String[]{"%.nomedia%"}, null);
    ...
    public static String extractProjectionFromXMPHeader(String filePath) {
        try {
            ...
            if ("equirectangular".equalsIgnoreCase(getXMPProperty(metaData, "http://ns.google.com/photos/1.0/panorama/", "GPano:ProjectionType"))) {
                return "EQUIRECTANGULAR";
            }
            String croppedWidthStr = getXMPProperty(metaData, "http://ns.google.com/photos/1.0/panorama/", "GPano:CroppedAreaImageWidthPixels");
            String panoWidthStr = getXMPProperty(metaData, "http://ns.google.com/photos/1.0/panorama/", "GPano:FullPanoWidthPixels");
    ...
    public static String extractAudioMimeTypeFromXMPHeader(String filePath) {
        try {
            ...
            return getXMPProperty(XMPMetaFactory.parseFromString(xmpString), "http://ns.google.com/photos/1.0/audio/", "GAudio:Mime");
        } catch (XMPException e) {
            Log.e(LocalMediaManager.TAG, "XMP parse failure for " + filePath);
            return null;
        }
    ...
    public static byte[] extractAudioFromXMPHeader(String filePath) {
        ...
        try {
            ...
            while (true) {
                ...
                if (offSet == -1) {
                    break;
                } else if (offSet == 255 && inputStream.read() == 225 && (segment = readJPEGSegment(inputStream, XMP_EXTENSION_NAMESPACE_BYTES)) != null) {
                    if (!started) {
    ...
    public static int getImageOrientationDegrees(String filePath, String mimeType) {
        if (mimeType != null && mimeType.compareTo("image/jpeg") != 0) {
            return 0;
        }
        try {
    ...
    private static String getJPEGXMP(File inputFile) {
        try {
            ...
            while (true) {
                try {
                    int offSet = inputStream.read();
                    if (offSet == -1 || (offSet == 255 && inputStream.read() == 225 && (result = checkJPEGNamespace(inputStream, "http://ns.adobe.com/xap/1.0/")) != null)) {
                        break;
                    }
                } catch (IOException e) {
