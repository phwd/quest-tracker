...
public class DeviceConfigClient {
    ...
    public Boolean debugOnlyGetDeviceBoolean(String paramName) {
        try {
            if (!this.mSubscribeSucceeded.get()) {
                BLog.e(TAG, "debugOnlyGetDeviceBoolean: Calling debugOnlyGetDeviceBoolean(%s) when not subscribed.", paramName);
                return null;
                ...
                BLog.e(TAG, "debugOnlyGetDeviceBoolean: Calling debugOnlyGetDeviceBoolean(%s) when mobile config client is not ready", paramName);
                return null;
            }
        } catch (Exception e) {
            BLog.e(TAG, "debugOnlyGetDeviceBoolean: Calling debugOnlyGetDeviceBoolean(%s) and got exception", paramName, e);
            return null;
        }
    }
    ...
    public Double debugOnlyGetDeviceDouble(String paramName) {
        try {
            if (!this.mSubscribeSucceeded.get()) {
                BLog.e(TAG, "debugOnlyGetDeviceDouble: Calling debugOnlyGetDeviceDouble(%s) when not subscribed.", paramName);
                return null;
            } else if (this.mMobileConfigClient != null) {
                ...
                BLog.e(TAG, "debugOnlyGetDeviceDouble: Calling debugOnlyGetDeviceDouble(%s) when mobile config client is not ready", paramName);
                return null;
            }
        } catch (Exception e) {
            BLog.e(TAG, "debugOnlyGetDeviceDouble: Calling debugOnlyGetDeviceDouble(%s) and got exception", paramName, e);
            return null;
        }
    }
    ...
    public Long debugOnlyGetDeviceLong(String paramName) {
        try {
            if (!this.mSubscribeSucceeded.get()) {
                BLog.e(TAG, "debugOnlyGetDeviceLong: Calling debugOnlyGetDeviceLong(%s) when not subscribed.", paramName);
                return null;
            } else if (this.mMobileConfigClient != null) {
                ...
                BLog.e(TAG, "debugOnlyGetDeviceLong: Calling debugOnlyGetDeviceLong(%s) when mobile config client is not ready", paramName);
                return null;
            }
        } catch (Exception e) {
            BLog.e(TAG, "debugOnlyGetDeviceLong: Calling debugOnlyGetDeviceLong(%s) and got exception", paramName, e);
            return null;
        }
    }
    ...
    public String debugOnlyGetDeviceString(String paramName) {
        try {
            if (!this.mSubscribeSucceeded.get()) {
                BLog.e(TAG, "debugOnlyGetDeviceString: Calling debugOnlyGetDeviceString(%s) when not subscribed.", paramName);
                return null;
            } else if (this.mMobileConfigClient != null) {
                return this.mMobileConfigClient.getStringWithOptions(paramName, getStringDefault(paramName), this.mMobileConfigOptions);
            } else {
                BLog.e(TAG, "debugOnlyGetDeviceString: Calling debugOnlyGetDeviceString(%s) when mobile config client is not ready", paramName);
                return null;
            }
        } catch (Exception e) {
            BLog.e(TAG, "debugOnlyGetDeviceString: Calling debugOnlyGetDeviceString(%s) and got exception", paramName, e);
            return null;
        }
    }
    ...
    private Boolean debugOnlyParseAsBoolean(String valueStr) {
    ...
    private Double debugOnlyParseAsDouble(String valueStr) {
    ...
    private Long debugOnlyParseAsLong(String valueStr) {
        if (valueStr != null && debugOnlyCheckValueStr(valueStr)) {
    ...
    private String debugOnlyParseAsString(String valueStr) {
        if (debugOnlyCheckValueStr(valueStr)) {
            return valueStr;
        }
        return null;
    ...
    private boolean debugOnlyCheckValueStr(String valueStr) {
                ...
                try {
                    ...
                    if (line == null) {
                        ...
                        if (!line.equals("DeviceConfig,params_default.txt,V1")) {
                            BLog.e(TAG, "Unexpected header '%s' for default param file", line);
                            bufferedReader.close();
                            return;
                        }
                        checkHeader = false;
                        ...
                        if (splitLine.length == 2) {
                            ...
                            if (entry != null) {
                                ...
                                switch (paramType) {
                                    ...
                                    default:
                                        try {
                                            BLog.e(TAG, "Unsupported param type %s", Integer.valueOf(paramType));
                                            continue;
                                ...
                                BLog.e(TAG, "Can't set default for unknown param '%s'", configParamName);
                            }
                        } else {
                            BLog.e(TAG, "Incorrect default format for param '%s'", line);
                        }
                    }
                } catch (Throwable th) {
                    th.addSuppressed(th);
    ...
    private Object getParamDefault(String paramName) {
        ParamsMapEntry entry = this.mParamsMapEntries.get(paramName);
        if (entry == null) {
            BLog.e(TAG, "Cannot get default value for unsupported param name %s", paramName);
            return null;
        }
        int paramType = entry.paramType;
        switch (paramType) {
            ...
            default:
                BLog.e(TAG, "Unsupported param type %s for param name %s", Integer.valueOf(paramType), paramName);
                return null;
        }
    }
    ...
    private <Type> void updateMemoryStateValueFromServiceInternal(Map<String, ValueInfo<Type>> valuesToUpdate, String paramName, Type value, CacheUpdateMode cacheUpdateMode, String loggingId, boolean sessionless) {
        ...
        switch (cacheUpdateMode) {
            case ReturnedValueAndSerialization:
                serializationNeeded = updatedValue.setValueFromService(value, true);
                break;
            case InMemoryAndSerialization:
