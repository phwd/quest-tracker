...
public class NativeCrashDumpReporterUtil {
    ...
    public static boolean processFatMinidump(Context context, CrashReportData crashReportData, Spool.FileBeingConsumed fileBeingConsumed) {
        File file = new File(fileBeingConsumed.fileName.getParentFile(), AnonymousClass08.A05(fileBeingConsumed.fileName.getName().split("_")[0], "_", EXTRA_MAP_SUFFIX));
        if (file.exists()) {
    ...
    public static boolean processCoreDump(Context context, CrashReportData crashReportData, Spool.FileBeingConsumed fileBeingConsumed) {
        String str;
        ...
        if (file.exists() && file.length() != 0) {
            ...
            if (file2.exists() && Math.abs(file.lastModified() - fileBeingConsumed.fileName.lastModified()) <= LigerHttpResponseHandler.MAX_WAIT_TIME_MILLIS) {
                long currentTimeMillis = System.currentTimeMillis();
    ...
    public static boolean processHeapDump(Context context, CrashReportData crashReportData, Spool.FileBeingConsumed fileBeingConsumed, AcraReportingConfig acraReportingConfig) {
        String str;
    ...
    public static void updateProcessedTime(File file, long j) {
        if (!file.exists()) {
