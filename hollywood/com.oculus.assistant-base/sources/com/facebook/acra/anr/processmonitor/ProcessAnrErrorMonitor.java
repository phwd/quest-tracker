...
public class ProcessAnrErrorMonitor {
    ...
    public class MonitorThread extends Thread {
        ...
        private void maybeLogAnrStateFromOtherProcesses(LinkedList linkedList, String str) {
            if (this.mListener != null) {
                ...
                while (it.hasNext()) {
                    ...
                    if (!str.equals(str2)) {
                        C0139Dd.A0P(ProcessAnrErrorMonitor.LOG_TAG, "Error found in process '%s' different from process being searched '%s'", str2, str);
        ...
        public MonitorThread(ActivityManager activityManager, ProcessErrorStateListener processErrorStateListener, long j, long j2) {
            super(AnonymousClass08.A04("ProcessAnrErrorMonitorThread:", "unknown"));
    ...
    public void startMonitoringAfterDelay(ProcessErrorStateListener processErrorStateListener, long j) {
        C0139Dd.A0P(LOG_TAG, "startMonitoring with delay: %d", Long.valueOf(j));
        ActivityManager activityManager = (ActivityManager) this.mContext.getSystemService(AppComponentStats.TAG_ACTIVITY);
        synchronized (this) {
