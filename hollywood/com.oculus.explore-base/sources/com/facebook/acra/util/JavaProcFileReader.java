    ...
    public String getOpenFileDescriptors() {
        ...
        try {
            ...
            for (int i = 1; i < lines.length; i++) {
                ...
                if (index != -1) {
                    String fileLeafPath = line.substring(" -> ".length() + index);
                    int index2 = fileLeafPath.lastIndexOf("/fd/");
                    if (index2 == -1) {
                        shortFileLeafPath = fileLeafPath;
                    } else {
                        shortFileLeafPath = fileLeafPath.substring("/fd/".length() + index2);
                    }
                    if (shortFileLeafPath.startsWith("pipe")) {
                        fileLeafPath = "pipe";
                    } else if (shortFileLeafPath.startsWith("socket")) {
                        fileLeafPath = "socket";
                    }
