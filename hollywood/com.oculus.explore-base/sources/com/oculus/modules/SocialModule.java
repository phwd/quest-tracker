...
public class SocialModule extends RCTBaseJavaModule {
    ...
    public static List<Pair<String, String>> describe() {
        ...
        list.add(new Pair<>("checkFBConnectAccount", BuildConfig.PROVIDER_SUFFIX));
    ...
    public void fetchFBConnectAccount(final int callbackID) {
        ThreadExecutor.getInstance().execute(new Callable<String>() {
            ...
            public String call() {
                String facebookAccountString = "null";
                Cursor cursor = null;
                try {
                    Cursor cursor2 = SocialModule.mFBConnectContentResolver.query(SocialModule.HORIZON_CONTENT_URI, new String[]{SocialModule.HORIZON_UID_KEY, SocialModule.HORIZON_USER_NAME_KEY, SocialModule.HORIZON_ACCESS_TOKEN_KEY, SocialModule.HORIZON_PROFILE_PICTURE_KEY}, null, null, null);
                    if (cursor2 != null) {
                        int userIdColumnIndex = cursor2.getColumnIndex(SocialModule.HORIZON_UID_KEY);
                        int userNameColumnIndex = cursor2.getColumnIndex(SocialModule.HORIZON_USER_NAME_KEY);
                        int accessTokenColumnIndex = cursor2.getColumnIndex(SocialModule.HORIZON_ACCESS_TOKEN_KEY);
                        int profilePictureColumnIndex = cursor2.getColumnIndex(SocialModule.HORIZON_PROFILE_PICTURE_KEY);
            ...
            public void onSuccess(String facebookAccountString) {
