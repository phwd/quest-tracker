...
public class PrecomputedTextCompat implements Spannable {
    ...
    public static PrecomputedTextCompat create(@NonNull CharSequence text, @NonNull Params params) {
        ...
        if (Build.VERSION.SDK_INT >= 23) {
            StaticLayout.Builder.obtain(text, 0, text.length(), params.getTextPaint(), DataConnectionPowerStateEnum.DATA_CONNECTION_POWER_STATE_UNKNOWN_VALUE).setBreakStrategy(params.getBreakStrategy()).setHyphenationFrequency(params.getHyphenationFrequency()).setTextDirection(params.getTextDirection()).build();
        } else if (Build.VERSION.SDK_INT >= 21) {
            new StaticLayout(text, params.getTextPaint(), DataConnectionPowerStateEnum.DATA_CONNECTION_POWER_STATE_UNKNOWN_VALUE, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
        }
