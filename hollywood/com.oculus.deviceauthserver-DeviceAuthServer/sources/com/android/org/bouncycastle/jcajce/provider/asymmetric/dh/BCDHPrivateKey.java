...
public class BCDHPrivateKey implements DHPrivateKey, PKCS12BagAttributeCarrier {
    ...
    public BCDHPrivateKey(PrivateKeyInfo info2) throws IOException {
        ...
        this.info = info2;
        this.x = ((ASN1Integer) info2.parsePrivateKey()).getValue();
    ...
    public byte[] getEncoded() {
        ...
        try {
            ...
            if (!(this.dhSpec instanceof DHDomainParameterSpec) || ((DHDomainParameterSpec) this.dhSpec).getQ() == null) {
                info2 = new PrivateKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.dhKeyAgreement, new DHParameter(this.dhSpec.getP(), this.dhSpec.getG(), this.dhSpec.getL()).toASN1Primitive()), new ASN1Integer(getX()));
            } else {
                ...
                info2 = new PrivateKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.dhpublicnumber, new DomainParameters(params.getP(), params.getG(), params.getQ(), params.getJ(), vParams).toASN1Primitive()), new ASN1Integer(getX()));
            }
            return info2.getEncoded(ASN1Encoding.DER);
        } catch (Exception e) {
            return null;
        }
    }
