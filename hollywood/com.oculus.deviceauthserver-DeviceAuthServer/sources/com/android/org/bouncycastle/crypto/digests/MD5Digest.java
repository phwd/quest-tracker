...
public class MD5Digest extends GeneralDigest implements EncodableDigest {
    ...
    public void processWord(byte[] in, int inOff) {
        ...
        int i = this.xOff;
        this.xOff = i + 1;
        iArr[i] = (in[inOff] & UnsignedBytes.MAX_VALUE) | ((in[inOff + 1] & UnsignedBytes.MAX_VALUE) << 8) | ((in[inOff + 2] & UnsignedBytes.MAX_VALUE) << 16) | ((in[inOff + 3] & UnsignedBytes.MAX_VALUE) << 24);
        if (this.xOff == 16) {
            processBlock();
        }
    }
