...
public final class CryptoServicesRegistrar {
    ...
    public static <T> T getSizedProperty(Property property, int size) {
        ...
        if (property.type.isAssignableFrom(DHParameters.class)) {
            for (int i = 0; i != values.length; i++) {
                T t = (T) ((DHParameters) values[i]);
                if (t.getP().bitLength() == size) {
                    return t;
                }
            }
        } else if (property.type.isAssignableFrom(DSAParameters.class)) {
            for (int i2 = 0; i2 != values.length; i2++) {
                T t2 = (T) ((DSAParameters) values[i2]);
                if (t2.getP().bitLength() == size) {
                    return t2;
                }
            }
        }
    ...
    private static void checkPermission(final Permission permission) {
        ...
        if (securityManager != null) {
            AccessController.doPrivileged(new PrivilegedAction<Object>() {
                /* class com.android.org.bouncycastle.crypto.CryptoServicesRegistrar.AnonymousClass1 */
    ...
    private static DHParameters toDH(DSAParameters dsaParams) {
        return new DHParameters(dsaParams.getP(), dsaParams.getG(), dsaParams.getQ(), chooseLowerBound(dsaParams.getP().bitLength()), 0, null, new DHValidationParameters(dsaParams.getValidationParameters().getSeed(), dsaParams.getValidationParameters().getCounter()));
    }
    ...
    public static final class Property {
        ...
        private Property(String name2, Class type2) {
            this.name = name2;
            this.type = type2;
        }
    }
