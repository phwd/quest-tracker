    ...
    private static Intent enforceExternalExtra(Intent intent, Reporter reporter, boolean z) {
        ...
        for (String str2 : arrayList) {
            if (!z) {
                ...
                reporter.report("ExternalIntentSanitization", String.format("Removed an internal class in a different-key intent: %s => %s", str2, extras.get(str2)), null);
            } else {
                reporter.report("ExternalIntentSanitization", String.format("Found an internal class in a different-key intent but not removing: %s => %s", str2, extras.get(str2)), null);
            }
        }
        return intent;
    }
