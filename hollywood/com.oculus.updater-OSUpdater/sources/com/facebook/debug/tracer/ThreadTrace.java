...
public final class ThreadTrace {
        ...
        try {
            if (getTracerCount() <= 2000) {
                ...
                if (realNanoTime2 > 1000000) {
                    BLog.d("ThreadTrace", "Pruned %d events on thread 0x%x in %d ms", Integer.valueOf(i), Long.valueOf(Thread.currentThread().getId()), Long.valueOf(realNanoTime2 / 1000000));
                    return;
                }
                return;
            }
            int pruneOldStoppedTracers = i + pruneOldStoppedTracers() + pruneFineGrainedTracers(1500);
            if (getTracerCount() > 1500) {
                pruneOldStoppedTracers += pruneComments(1500);
            }
            if (getTracerCount() > 2000) {
                BLog.wtf("ThreadTrace", "Resetting because hit couldn't get under hard limit after normal pruning");
                reset();
            } else if (getTracerCount() > 1500) {
            ...
            if (realNanoTime3 > 1000000 || i > 0) {
                BLog.d("ThreadTrace", "Pruned %d events on thread 0x%x in %d ms", Integer.valueOf(i), Long.valueOf(Thread.currentThread().getId()), Long.valueOf(realNanoTime3 / 1000000));
            }
        }
    }
    ...
    public int pruneOldStoppedTracers() {
        ...
        BLog.d("ThreadTrace", "Pruned %d old events", Integer.valueOf(i4));
        return i4;
    }
    ...
    public int pruneComments(int i) {
        ...
        BLog.d("ThreadTrace", "Pruned %d comments", Integer.valueOf(i2));
        return i2;
    }
