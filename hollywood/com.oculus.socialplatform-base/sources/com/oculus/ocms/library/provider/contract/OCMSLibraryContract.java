...
public class OCMSLibraryContract {
    ...
    public static String getFilenameFromAssetsUri(Uri uri) {
        String uriPathSegment = getUriPathSegment(uri, 3);
    ...
    public static long getIdFromAssetsUri(Uri uri) {
        ...
        if (!TextUtils.isEmpty(uriPathSegment)) {
            try {
                return Long.parseLong(uriPathSegment);
            } catch (NumberFormatException unused) {
                return -1;
            }
        } else {
    ...
    public static Uri providerUri() {
        return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).build();
    }

    public static Uri queryForDeploymentGk() {
        return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path(DEPLOYMENT_PATH).build();
    }

    public static Uri uriForAllDownloads() {
        return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path(DOWNLOAD_PATH).build();
    }

    public static Uri uriForAllInstalls() {
        return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path(INSTALL_PATH).build();
    }

    public static Uri uriForAllPackages() {
        return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path("apps").build();
    }

    public static Uri getAllAssetsUri(String str) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path("assets").appendPath(str).build();
        }
        throw new IllegalArgumentException("packageName cannot be null/empty");
    }

    public static Uri getAssetByFilenameUri(String str, String str2) {
        if (TextUtils.isEmpty(str)) {
            throw new IllegalArgumentException("packageName cannot be null/empty");
        } else if (!TextUtils.isEmpty(str2)) {
            return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path("assets").appendPath(str).appendPath(ASSETS_PATH_BY_FILENAME).appendPath(str2).build();
        } else {
            throw new IllegalArgumentException("filename cannot be null/empty");
        }
    }

    public static Uri getAssetByIdUri(String str, long j) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path("assets").appendPath(str).appendPath("id").appendPath(Long.toString(j)).build();
        }
        throw new IllegalArgumentException("packageName cannot be null/empty");
    }
    ...
    public static Uri uriForPackage(String str) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path("apps").appendPath(str).build();
        }
        throw new IllegalArgumentException("packageName cannot be null/empty");
    }
    ...
    public static Uri uriForPackageDownload(InstallerResult installerResult) {
        ...
        if (installerResultError == null) {
            ...
            i = installerResultError.code;
        }
        return uriForPackageDownload(installerResult.installIdentifier, i, installerResult.timestampMs);
    }

    public static Uri uriForPackageDownload(String str, int i, long j) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path(DOWNLOAD_PATH).appendPath(str).appendPath(Integer.toString(i)).appendPath(Long.toString(j)).build();
        }
    ...
    public static Uri uriForPackageInstall(InstallerResult installerResult) {
        ...
        return uriForPackageInstall(installerResult.installIdentifier, i, installerResult.timestampMs);
    }

    public static Uri uriForPackageInstall(String str, int i, long j) {
        if (!TextUtils.isEmpty(str)) {
            return new Uri.Builder().scheme(LoggingKeys.REFERRER_CONTENT).authority(AUTHORITY).path(INSTALL_PATH).appendPath(str).appendPath(Integer.toString(i)).appendPath(Long.toString(j)).build();
        }
