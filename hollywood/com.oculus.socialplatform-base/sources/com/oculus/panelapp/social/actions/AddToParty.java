...
public class AddToParty extends SocialUserAction {
    public static final String TAG = LoggingUtil.tag(AddToParty.class);
    ...
    public boolean isRelevant() {
        ...
        SocialParty socialParty = this.mUser.mParty;
        if ((partyMembership == null || partyMembership == SocialParty.PartyMembership.NONE) && socialParty != null) {
            return true;
        }
        return false;
    }
    ...
    public void preformAction(final SocialPanelApp socialPanelApp, final Context context, final SocialUserActionActionHandler socialUserActionActionHandler) {
        ...
        if (socialParty == null) {
            SocialLogger.logError(context, "invite_users_to_party", TAG, "Failed to invite users to party because the party was null.");
        } else {
            HorizonContentProviderHelper.inviteUsersToParty(context, arrayList, socialParty.mID, new HorizonContentProviderHelper.MultipleIDCallback() {
                /* class com.oculus.panelapp.social.actions.AddToParty.AnonymousClass1 */

                @Override // com.oculus.horizoncontent.horizon.HorizonContentProviderHelper.BaseCallback
                public void onError() {
                    socialUserActionActionHandler.onError();
                    SocialLogger.logError(context, "invite_users_to_party", AddToParty.TAG, "Failed to invite users to party");
