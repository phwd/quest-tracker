...
public class ANRReport implements IANRReport {
    ...
    public static class UUIDFileGenerator implements FileGenerator {
        ...
        public File generate() {
            return new File(this.mContext.getDir(this.mDirectory, 0), AnonymousClass006.A07(UUID.randomUUID().toString(), this.mExtension));
        }
    ...
    public static boolean deleteFile(File file) {
        ...
        AnonymousClass0MD.A0A(LOG_TAG, "Could not delete error report: %s", file.getName());
        return delete;
    }
    ...
    private void initializeProcessErrorPropertiesOnErrorReport() {
        synchronized (this.mANRProcessErrorProperties) {
            ErrorReporter.putCustomDataInternal(ErrorReportingConstants.ANR_PROCESS_ERROR_DETECTED, this.mANRProcessErrorProperties.get(ErrorReportingConstants.ANR_PROCESS_ERROR_DETECTED));
            ErrorReporter.putCustomDataInternal(ErrorReportingConstants.ANR_PROCESS_ERROR_DETECTION_FAILURE_TIME, this.mANRProcessErrorProperties.get(ErrorReportingConstants.ANR_PROCESS_ERROR_DETECTION_FAILURE_TIME));
            ErrorReporter.putCustomDataInternal(ErrorReportingConstants.ANR_PROCESS_ERROR_DETECTION_FAILURE_CAUSE, this.mANRProcessErrorProperties.get(ErrorReportingConstants.ANR_PROCESS_ERROR_DETECTION_FAILURE_CAUSE));
            ErrorReporter.putCustomDataInternal(ErrorReportingConstants.ANR_SYSTEM_ERROR_MSG, this.mANRProcessErrorProperties.get(ErrorReportingConstants.ANR_SYSTEM_ERROR_MSG));
            ErrorReporter.putCustomDataInternal(ErrorReportingConstants.ANR_SYSTEM_TAG, this.mANRProcessErrorProperties.get(ErrorReportingConstants.ANR_SYSTEM_TAG));
            ErrorReporter.putCustomDataInternal(ErrorReportingConstants.ANR_PROCESS_ERROR_DETECTION_START_TIME, this.mANRProcessErrorProperties.get(ErrorReportingConstants.ANR_PROCESS_ERROR_DETECTION_START_TIME));
            ErrorReporter.putCustomDataInternal(ErrorReportingConstants.ANR_MAIN_THREAD_UNBLOCKED_UPTIME, this.mANRProcessErrorProperties.get(ErrorReportingConstants.ANR_MAIN_THREAD_UNBLOCKED_UPTIME));
            ErrorReporter.putCustomDataInternal(ErrorReportingConstants.ANR_AM_CONFIRMATION_EXPIRED_UPTIME, this.mANRProcessErrorProperties.get(ErrorReportingConstants.ANR_AM_CONFIRMATION_EXPIRED_UPTIME));
            for (int i = 1; i <= this.mMaxUsedAnrProcessStateIndex; i++) {
                ErrorReporter.putCustomDataInternal(AnonymousClass006.A03(ErrorReportingConstants.ANR_OTHER_PROCESS_ERROR_PREFIX, i), this.mANRProcessErrorProperties.get(AnonymousClass006.A03(ErrorReportingConstants.ANR_OTHER_PROCESS_ERROR_PREFIX, i)));
            }
            this.mANRProcessErrorProperties.clear();
        }
    }
    ...
    public void logOtherProcessAnr(String str, @Nullable String str2, @Nullable String str3, long j) {
        synchronized (this.mANRProcessErrorProperties) {
            ...
            if (i < 5) {
                ...
                sb.append(',');
                sb.append(str3);
                this.mANRProcessErrorProperties.put(AnonymousClass006.A03(ErrorReportingConstants.ANR_OTHER_PROCESS_ERROR_PREFIX, i), sb.toString());
                addProcessErrorPropertiesToErrorReport();
        ...
        synchronized (this.mANRProcessErrorProperties) {
            ...
            if (!(str == null && str2 == null)) {
                try {
                    ...
                    AnonymousClass0MD.A07(LOG_TAG, "Failed to save SIGQUIT", e);
                }
            }
            addProcessErrorPropertiesToErrorReport();
        }
