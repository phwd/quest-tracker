        ...
        for (Field field : fields) {
            if (Modifier.isStatic(field.getModifiers()) && Modifier.isFinal(field.getModifiers())) {
                ...
                try {
                    ...
                    AnonymousClass0MD.A0D("ACRA", e, "Error while inspecting device configuration: ");
                }
            }
        }
    }
    ...
    public static String toString(Configuration configuration) {
        ...
        for (Field field : fields) {
            try {
                ...
                AnonymousClass0MD.A0C("ACRA", e, "Error while inspecting device configuration: ");
            }
        }
        return sb.toString();
    }
    ...
    public static String getFieldValueName(Configuration configuration, Field field) throws IllegalArgumentException, IllegalAccessException {
        ...
        if (!name.equals(FIELD_MCC) && !name.equals(FIELD_MNC)) {
            if (name.equals(FIELD_UIMODE)) {
                ...
                SparseArray<String> sparseArray = mValueArrays.get(AnonymousClass006.A01(name.toUpperCase(), '_'));
