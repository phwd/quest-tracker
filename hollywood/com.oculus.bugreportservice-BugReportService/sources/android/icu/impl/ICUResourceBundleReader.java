...
public final class ICUResourceBundleReader {
    ...
    /* access modifiers changed from: private */
    public static class ReaderCache extends SoftCache {
        private ReaderCache() {
        }
        ...
        public ICUResourceBundleReader createInstance(ReaderCacheKey readerCacheKey, ClassLoader classLoader) {
            ...
            try {
                if (readerCacheKey.baseName == null || !readerCacheKey.baseName.startsWith("android/icu/impl/data/icudt63b")) {
                    ...
                    byteBuffer = ICUBinary.getData(classLoader, fullName, fullName.substring(31));
                    if (byteBuffer == null) {
                        return ICUResourceBundleReader.NULL_READER;
                    }
                }
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void setKeyFromKey16(int i, UResource$Key uResource$Key) {
        int i2 = this.localKeyLimit;
        if (i < i2) {
            uResource$Key.setBytes(this.keyBytes, i);
        } else {
            uResource$Key.setBytes(this.poolBundleReader.keyBytes, i - i2);
        }
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void setKeyFromKey32(int i, UResource$Key uResource$Key) {
        if (i >= 0) {
            uResource$Key.setBytes(this.keyBytes, i);
        } else {
            uResource$Key.setBytes(this.poolBundleReader.keyBytes, i & Integer.MAX_VALUE);
        }
    }
    ...
    public Table getTable(int i) {
        ...
        if (RES_GET_TYPE == 2) {
            container = new Table1632(this, RES_GET_OFFSET);
            size = container.getSize();
        } else if (RES_GET_TYPE == 5) {
            container = new Table16(this, RES_GET_OFFSET);
            size = container.getSize();
        } else {
            container = new Table32(this, RES_GET_OFFSET);
            i2 = container.getSize() * 4;
            return (Table) this.resourceCache.putIfAbsent(i, container, i2);
        }
        i2 = size * 2;
        return (Table) this.resourceCache.putIfAbsent(i, container, i2);
    }
    ...
    public static class Table extends Container implements UResource$Table {
        ...
        public boolean getKeyAndValue(int i, UResource$Key uResource$Key, UResource$Value uResource$Value) {
            ...
            char[] cArr = this.keyOffsets;
            if (cArr != null) {
                readerValue.reader.setKeyFromKey16(cArr[i], uResource$Key);
            } else {
                readerValue.reader.setKeyFromKey32(this.key32Offsets[i], uResource$Key);
            }
            readerValue.res = getContainerResource(readerValue.reader, i);
            return true;
        }
